...
arch=$(uname -m)
dist=el$(cat /etc/redhat-release | cut -d " " -f4 | cut -d "." -f1)
...
mkdir -p /root/rpmbuild/BUILDROOT/openamdc-$version-$release.$dist.$arch
cp -r $DIR/openamdc_build/openamdc_rpm/* /root/rpmbuild/BUILDROOT/openamdc-$version-$release.$dist.$arch/
