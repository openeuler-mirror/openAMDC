...
start_server {tags {"benchmark network"}} {
    start_server {} {
        ...
        test {benchmark: set,get} {
            r config resetstat
            r flushall
            set cmd [openamdcbenchmark $master_host $master_port "-c 5 -n 10 -e -t set,get"]
            if {[catch { exec {*}$cmd } error]} {
                set first_line [lindex [split $error "\n"] 0]
                puts [colorstr red "openamdc-benchmark non zero code. first line: $first_line"]
                fail "openamdc-benchmark non zero code. first line: $first_line"
            }
        ...
        test {benchmark: full test suite} {
            r config resetstat
            set cmd [openamdcbenchmark $master_host $master_port "-c 10 -n 100 -e"]
            if {[catch { exec {*}$cmd } error]} {
                set first_line [lindex [split $error "\n"] 0]
                puts [colorstr red "openamdc-benchmark non zero code. first line: $first_line"]
                fail "openamdc-benchmark non zero code. first line: $first_line"
        ...
        test {benchmark: multi-thread set,get} {
            r config resetstat
            r flushall
            set cmd [openamdcbenchmark $master_host $master_port "--threads 10 -c 5 -n 10 -e -t set,get"]
            if {[catch { exec {*}$cmd } error]} {
                set first_line [lindex [split $error "\n"] 0]
                puts [colorstr red "openamdc-benchmark non zero code. first line: $first_line"]
                fail "openamdc-benchmark non zero code. first line: $first_line"
        ...
        test {benchmark: pipelined full set,get} {
            r config resetstat
            r flushall
            set cmd [openamdcbenchmark $master_host $master_port "-P 5 -c 10 -n 10010 -e -t set,get"]
            if {[catch { exec {*}$cmd } error]} {
                set first_line [lindex [split $error "\n"] 0]
                puts [colorstr red "openamdc-benchmark non zero code. first line: $first_line"]
                fail "openamdc-benchmark non zero code. first line: $first_line"
        ...
        test {benchmark: arbitrary command} {
            r config resetstat
            r flushall
            set cmd [openamdcbenchmark $master_host $master_port "-c 5 -n 150 -e INCRBYFLOAT mykey 10.0"]
            if {[catch { exec {*}$cmd } error]} {
                set first_line [lindex [split $error "\n"] 0]
                puts [colorstr red "openamdc-benchmark non zero code. first line: $first_line"]
                fail "openamdc-benchmark non zero code. first line: $first_line"
        ...
        test {benchmark: keyspace length} {
            r flushall
            r config resetstat
            set cmd [openamdcbenchmark $master_host $master_port "-r 50 -t set -n 1000"]
            if {[catch { exec {*}$cmd } error]} {
                set first_line [lindex [split $error "\n"] 0]
                puts [colorstr red "openamdc-benchmark non zero code. first line: $first_line"]
                fail "openamdc-benchmark non zero code. first line: $first_line"
        ...
        if {$::tls} {
            test {benchmark: specific tls-ciphers} {
                r flushall
                r config resetstat
                set cmd [openamdcbenchmark $master_host $master_port "-r 50 -t set -n 1000 --tls-ciphers \"DEFAULT:-AES128-SHA256\""]
                if {[catch { exec {*}$cmd } error]} {
                    set first_line [lindex [split $error "\n"] 0]
                    puts [colorstr red "openamdc-benchmark non zero code. first line: $first_line"]
                    fail "openamdc-benchmark non zero code. first line: $first_line"
            ...
            test {benchmark: specific tls-ciphersuites} {
                ...
                set cmd [openamdcbenchmark $master_host $master_port "-r 50 -t set -n 1000 --tls-ciphersuites \"TLS_AES_128_GCM_SHA256\""]
                if {[catch { exec {*}$cmd } error]} {
                    set first_line [lindex [split $error "\n"] 0]
