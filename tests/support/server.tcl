...
proc start_server_error {config_file error} {
    set err {}
    append err "Can't start the openAMDC server\n"
...
proc spawn_server {config_file stdout stderr} {
    if {$::valgrind} {
        set pid [exec valgrind --track-origins=yes --trace-children=yes --suppressions=[pwd]/src/valgrind.sup --show-reachable=no --show-possibly-lost=no --leak-check=full src/openamdc-server $config_file >> $stdout 2>> $stderr &]
    } elseif ($::stack_logging) {
        set pid [exec /usr/bin/env MallocStackLogging=1 MallocLogFile=/tmp/malloc_log.txt src/openamdc-server $config_file >> $stdout 2>> $stderr &]
    } else {
        set pid [exec src/openamdc-server $config_file >> $stdout 2>> $stderr &]
    }
...
proc start_server {options {code undefined}} {
    ...
    if {$::tls} {
        ...
        dict set config "tls-dh-params-file" [format "%s/tests/tls/openamdc.dh" [pwd]]
    ...
    set config_file [tmpfile openamdc.conf]
