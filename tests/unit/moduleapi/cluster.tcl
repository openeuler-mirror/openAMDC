# Primitive tests on cluster-enabled openAMDC with modules using openamdc-cli
...
start_server [list overrides $base_conf] {
    ...
    test {Create 3 node cluster} {
        exec src/openamdc-cli --cluster-yes --cluster create \
    ...
    test "Perform a Resharding" {
        exec src/openamdc-cli --cluster-yes --cluster reshard 127.0.0.1:[srv -2 port] \
    ...
    test "Wait for cluster to be stable" {
        wait_for_condition 1000 50 {
            [catch {exec src/openamdc-cli --cluster \
