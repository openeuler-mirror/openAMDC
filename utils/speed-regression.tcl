...
# Released under the BSD license like openamdc itself
...
proc run-tests branches {
    ...
    foreach b $branches {
        ...
        if {$branch_id == 0} {
            puts "  copy openamdc-benchmark from unstable to /tmp..."
            exec -ignorestderr cp ./openamdc-benchmark /tmp
            incr branch_id
            continue
        }

        # Start the openAMDC server
        puts "  starting the server... [exec ./openamdc-server -v]"
        set pids [exec echo "port $::port\nloglevel warning\n" | ./openamdc-server - > /dev/null 2> /dev/null &]
        puts "  pids: $pids"
        after 1000
        ...
        set i [$r info]
        puts "  openamdc INFO shows version: [lindex [split $i] 0]"
        $r close

        set output [exec /tmp/openamdc-benchmark -n $::requests -t $::tests -d $::datasize --csv -p $::port]
...
proc main {} {
    # Note: the first branch is only used in order to get the openamdc-benchmark
