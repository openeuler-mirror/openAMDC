 ...
 * to avoid direct socket and async event management across the openAMDC code base.
 *
...
static int connSocketConnect(connection *conn, const char *addr, int port, const char *src_addr,
    ...
    aeCreateFileEvent(server.el[threadId], conn->fd, AE_WRITABLE|AE_WRITE_THREADSAFE,
...
static void connSocketClose(connection *conn) {
    if (conn->fd != -1) {
        aeDeleteFileEvent(server.el[threadId], conn->fd, AE_READABLE | AE_WRITABLE);
        close(conn->fd);
        conn->fd = -1;
    }
 ...
 * loop.
 */
static int connSocketSetWriteHandler(connection *conn, ConnectionCallbackFunc func, int barrier, int threadSafe) {
    ...
    if (!conn->write_handler)
        aeDeleteFileEvent(server.el[threadId],conn->fd,AE_WRITABLE);
    else
        if (aeCreateFileEvent(server.el[threadId],conn->fd,AE_WRITABLE|AE_WRITE_THREADSAFE,
 ...
 * If NULL, the existing handler is removed.
 */
static int connSocketSetReadHandler(connection *conn, ConnectionCallbackFunc func, int threadSafe) {
    ...
    if (!conn->read_handler)
        aeDeleteFileEvent(server.el[threadId],conn->fd,AE_READABLE);
    else
        if (aeCreateFileEvent(server.el[threadId],conn->fd,
...
static void connSocketEventHandler(struct aeEventLoop *el, int fd, void *clientData, int mask)
{
    ...
    if (conn->state == CONN_STATE_CONNECTING &&
        ...
        if (!conn->write_handler) aeDeleteFileEvent(server.el[threadId],conn->fd,AE_WRITABLE);
