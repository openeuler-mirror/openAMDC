 ...
 * in openAMDC. Another thread may be calling localtime() while the main thread
 * forks(). Later when the child process calls localtime() again, for instance
 * in order to log something to the openAMDC log, it may deadlock: in the copy
 ...
 * designed to work with what time(NULL) may return, and to support openAMDC
