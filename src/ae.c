...
__thread aeEventLoop *eventLoopThread = NULL;

struct aeAsyncTask {
    int lock;
    aeAsyncProc* proc;
    void *var;
    aeAsyncCallback *callback;
};

/* This function reads an asynchronous task command from a specified file descriptor
 * and executes the corresponding task processing function. */
static void aeAsyncTaskProcess(aeEventLoop *eventLoop, int fd, void *clientData, int mask) {
    AE_NOTUSED(eventLoop);
    AE_NOTUSED(clientData);
    AE_NOTUSED(mask);

    /* Defines a deferred lock mutex to ensure thread-safe access
     * to global resources. */
...
int aeResizeSetSize(aeEventLoop *eventLoop, int setsize) {
    assert(eventLoopThread == NULL || eventLoopThread == eventLoop);
    int i;
...
void aeDeleteEventLoop(aeEventLoop *eventLoop) {
    close(eventLoop->fdAsyncRead);
    close(eventLoop->fdAsyncWrite);
    aeApiFree(eventLoop);
    zfree(eventLoop->events);
    zfree(eventLoop->fired);
 ...
 * Possible optimizations (not needed by openAMDC so far, but...):
...
static int processTimeEvents(aeEventLoop *eventLoop) {
    ...
    while(te) {
        ...
        if (te->id == AE_DELETED_EVENT_ID) {
            ...
            if (te->finalizerProc) {
                WRAPPER_MUTEX_LOCK(wm, &globalLock);
        ...
        for (j = 0; j < numevents; j++) {
            ...
            if (fe->mask & mask & AE_WRITABLE) {
                if (!fired || fe->wfileProc != fe->rfileProc) {
                    WRAPPER_MUTEX_DEFER_LOCK(wm, &globalLock) ;
                    if (!(fe->mask & AE_WRITE_THREADSAFE))
                        wrapperMutexLock(&wm);
...
void aeClosePipes(aeEventLoop *eventLoop){
    close(eventLoop->fdAsyncRead);
    eventLoop->fdAsyncRead = -1;
    close(eventLoop->fdAsyncWrite);
    eventLoop->fdAsyncWrite = -1;
}