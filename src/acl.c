...
list *UsersToLoad;  /* This is a list of users found in the configuration file
                       that we'll need to load in the final stage of openAMDC
...
void ACLFreeUserAndKillClients(user *u) {
    ...
    while ((ln = listNext(&li)) != NULL) {
        ...
        if (c->user == u) {
             ...
             * However if there are bugs in openAMDC, soon or later
 ...
 *              the openAMDC command table is described and defined.
 ...
 * users in the openamdc.conf configuration or in the ACL file, that is:
 *
 *  user <username> ... rules ...
 *
 ...
 * the pending list of users defined in openamdc.conf, or from the ACL file.
 * The function will just exit with an error if the user is trying to mix
 * both the loading methods. */
void ACLLoadUsersAtStartup(void) {
    if (server.acl_filename[0] != '\0' && listLength(UsersToLoad) != 0) {
        serverLog(LL_WARNING,
            "Configuring openAMDC with users defined in openamdc.conf and at "
            "the same setting an ACL file path is invalid. This setup "
            "is very likely to lead to configuration errors and security "
            "holes, please define either an ACL file or declare users "
            "directly in your openamdc.conf, but not both.");
        exit(1);
    }
    ...
    if (server.acl_filename[0] != '\0') {
        ...
        if (errors) {
            serverLog(LL_WARNING,
                "Aborting openAMDC startup because of ACL errors: %s", errors);
            sdsfree(errors);
            exit(1);
        }
    }
...
void aclCommand(client *c) {
    ...
    {
        addReplyError(c,"This openAMDC instance is not configured to use an ACL file. You may want to specify users via the ACL SETUSER command and then issue a CONFIG REWRITE (assuming you have a openAMDC configuration file set) in order to store users in the openAMDC configuration.");
        return;
