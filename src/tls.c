...
static __thread list *pending_list = NULL;
...
int tlsConfigure(redisTLSContextConfig *ctx_config) {
    ...
    if (ctx_config->session_caching) {
        ...
        SSL_CTX_set_session_id_context(ctx, (void *) "openAMDC", 8);
    } else {
...
void registerSSLEvent(tls_connection *conn, WantIOType want) {
    serverAssert(conn->el == server.el[threadId]);
    int mask = aeGetFileEvents(server.el[threadId], conn->c.fd);

    switch (want) {
        case WANT_READ:
            if (mask & AE_WRITABLE) aeDeleteFileEvent(server.el[threadId], conn->c.fd, AE_WRITABLE);
            if (!(mask & AE_READABLE)) aeCreateFileEvent(server.el[threadId], conn->c.fd, AE_READABLE|AE_READ_THREADSAFE,
                        tlsEventHandler, conn);
            break;
        case WANT_WRITE:
            if (mask & AE_READABLE) aeDeleteFileEvent(server.el[threadId], conn->c.fd, AE_READABLE);
            if (!(mask & AE_WRITABLE)) aeCreateFileEvent(server.el[threadId], conn->c.fd, AE_WRITABLE|AE_WRITE_THREADSAFE,
                        tlsEventHandler, conn);
            break;
        default:
            serverAssert(0);
...
void updateSSLEventCore(tls_connection *conn) {
    ...
    if (need_read && !(mask & AE_READABLE))
        aeCreateFileEvent(server.el[threadId], conn->c.fd, AE_READABLE|AE_READ_THREADSAFE, tlsEventHandler, conn);
    if (!need_read && (mask & AE_READABLE))
        aeDeleteFileEvent(server.el[threadId], conn->c.fd, AE_READABLE);

    if (need_write && !(mask & AE_WRITABLE))
        aeCreateFileEvent(server.el[threadId], conn->c.fd, AE_WRITABLE|AE_WRITE_THREADSAFE, tlsEventHandler, conn);
    if (!need_write && (mask & AE_WRITABLE))
        aeDeleteFileEvent(server.el[threadId], conn->c.fd, AE_WRITABLE);
}

void asyncUpdateSSLEventCore(void *var, aeAsyncCallback *callback) {
    UNUSED(callback);
    tls_connection *conn = var;
    updateSSLEventCore(conn);
}

void updateSSLEvent(tls_connection *conn) {
    if (conn->el != server.el[threadId]) {
        aeAsyncFunction(conn->el, asyncUpdateSSLEventCore, conn, NULL, 1);
    } else {
        updateSSLEventCore(conn);
    }
}
...
static void tlsHandleEvent(tls_connection *conn, int mask) {
    ...
    switch (conn->c.state) {
        ...
        case CONN_STATE_CONNECTED:
        {
            ...
            if (!invert && call_read) {
                WRAPPER_MUTEX_DEFER_LOCK(gl, &globalLock);
...
void asyncConnTLSCloseCore(void *var, aeAsyncCallback *callback) {
    UNUSED(callback);
    tls_connection *conn = var;
...
static void connTLSClose(connection *conn_) {
    ...
    if (conn->el != server.el[threadId]) {
        aeAsyncFunction(server.el[threadId], asyncConnTLSCloseCore, conn, NULL, 1);
    } else {
        connTLSCloseCore(conn);
    }
}
...
int connTLSSetWriteHandler(connection *conn, ConnectionCallbackFunc func, int barrier, int threadSafe) {
...
int connTLSSetReadHandler(connection *conn, ConnectionCallbackFunc func, int threadSafe) {
