...
#define CONN_FLAG_READ_THREADSAFE   (1<<2)    /* Thread safe read */
#define CONN_FLAG_WRITE_THREADSAFE  (1<<3)    /* Thread safe write */
...
typedef struct ConnectionType {
    ...
    int (*set_write_handler)(struct connection *conn, ConnectionCallbackFunc handler, int barrier, int threadSafe);
    int (*set_read_handler)(struct connection *conn, ConnectionCallbackFunc handler, int threadSafe);
 ...
 */
static inline int connSetWriteHandler(connection *conn, ConnectionCallbackFunc func, int threadSafe) {
    return conn->type->set_write_handler(conn, func, 0, threadSafe);
}

/* Register a read handler, to be called when the connection is readable.
 * If NULL, the existing handler is removed.
 */
static inline int connSetReadHandler(connection *conn, ConnectionCallbackFunc func, int threadSafe) {
    return conn->type->set_read_handler(conn, func, threadSafe);
}
...
static inline int connSetWriteHandlerWithBarrier(connection *conn, ConnectionCallbackFunc func, int barrier, int threadSafe) {
    return conn->type->set_write_handler(conn, func, barrier, threadSafe);
}
