...
struct ldbState {
    ...
    list *traces; /* Messages about openAMDC commands executed since last stop.*/
    ...
    int luabp;  /* Stop at next line because openAMDC.breakpoint() was called. */
...
/* ---------------------------------------------------------------------------
 * OpenAMDC reply to Lua type conversion functions.
 * ------------------------------------------------------------------------- */

/* Take a openAMDC reply in the Redis protocol format and convert it into a
 * Lua type. Thanks to this function, and the introduction of not connected
 * clients, it is trivial to implement the redis() lua function.
 *
 * Basically we take the arguments, execute the openAMDC command in the context
 * of a non connected client, then take the generated reply and convert it
 * into a suitable Lua type. With this trick the scripting feature does not
 * need the introduction of a full openAMDC internals API. The script
 * is like a normal client that bypasses all the slow I/O paths.
 *
 * Note: in this function we do not do any sanity check as the reply is
 * generated by openAMDC directly. This allows us to go faster.
 *
...
char *redisProtocolToLuaType_Aggregate(lua_State *lua, char *reply, int atype) {
    ...
    if (server.lua_client[threadId]->resp == 2 || atype == '*') {
        p += 2;
        if (mbulklen == -1) {
            lua_pushboolean(lua,0);
            return p;
    ...
    } else if (server.lua_client[threadId]->resp == 3) {
...
/* ---------------------------------------------------------------------------
 * Lua reply to openAMDC reply conversion functions.
 * ------------------------------------------------------------------------- */

/* Reply to client 'c' converting the top element in the Lua stack to a
 * openAMDC reply. As a side effect the element is consumed from the stack.  */
...
         * Notice that we need, in the worst case, 4 elements because returning a map might
    ...
    case LUA_TSTRING:
        ...
        break;
    case LUA_TBOOLEAN:
        if (server.lua_client[threadId]->resp == 2)
...
int luaRedisGenericCommand(lua_State *lua, int raise_error) {
    ...
    client *c = server.lua_client[threadId];
    sds reply;
    c->iel = threadId;
                   ...
                   (argc < -cmd->arity)))
    {
        if (cmd)
            luaPushError(lua,
                "Wrong number of args calling openAMDC command From Lua script");
        else
            luaPushError(lua,"Unknown openAMDC command called from Lua script");
        goto cleanup;
    }
    c->cmd = c->lastcmd = cmd;
    ...
    if (cmd->flags & CMD_NOSCRIPT) {
        luaPushError(lua, "This openAMDC command is not allowed from scripts");
        goto cleanup;
    }
    ...
    /* If this is a openAMDC Cluster node, we need to make sure Lua is not
    ...
    /* Convert the result of the openAMDC command into a suitable Lua type.
    ...
    /* If the debugger is active, log the reply from openAMDC. */
    if (ldb.active && ldb.step)
...
int luaSetResp(lua_State *lua) {
    ...
    server.lua_client[threadId]->resp = resp;
    return 0;
}
 ...
 *
 * It can be called again multiple times during the lifetime of the openAMDC
...
void scriptingInit(int setup) {
    ...
    if (setup) {
        for (int iel = 0; iel < server.worker_threads_num; ++iel) {
            server.lua_client[iel] = createClient(NULL, iel);
            server.lua_client[iel]->flags |= CLIENT_LUA;
            server.lua_client[iel]->flags |= CLIENT_DENY_BLOCKING;
        }
    ...
    /* Register the openAMDC commands table and fields */
    lua_newtable(lua);
...
/* Set an array of openAMDC String Objects as a Lua array (table) stored into a
 * global variable. */
...
sds luaCreateFunction(client *c, lua_State *lua, robj *body) {
    ...
    serverAssertWithInfo(c ? c : server.lua_client[threadId],NULL,retval == DICT_OK);
...
void luaMaskCountHook(lua_State *lua, lua_Debug *ar) {
    ...
    if (server.lua_timedout) processEventsWhileBlocked(threadId);
    if (server.lua_kill) {
...
void prepareLuaClient(void) {
    ...
    selectDb(server.lua_client[threadId],server.lua_caller->db->id);
    server.lua_client[threadId]->resp = 2; /* Default is RESP2, scripts can change it. */

    /* If we are in MULTI context, flag Lua client as CLIENT_MULTI. */
    if (server.lua_caller->flags & CLIENT_MULTI) {
        server.lua_client[threadId]->flags |= CLIENT_MULTI;
    }
}
 ...
 * way we don't have to re-enter openAMDC in order to handle I/O.
 *
...
int ldbStartSession(client *c) {
    ...
    if (ldb.forked) {
        ...
        if (cp == -1) {
             ...
             * to the clients. */
            serverLog(LL_WARNING,"openAMDC forked for debugging eval");
        } else {
            /* Parent */
        ...
        serverLog(LL_WARNING,
            "openAMDC synchronous debugging eval session started");
    }
...
void ldbEndSession(client *c) {
    ...
    if (ldb.forked) {
        ...
        serverLog(LL_WARNING,
            "openAMDC synchronous debugging eval session ended");
    }
 ...
 * take care of a given openAMDC return type. */
...
/* Log a openAMDC reply as debugger output, in a human readable format.
...
ldbLog(sdsnew("openAMDC Lua debugger help:"));
...
ldbLog(sdsnew("[r]edis <cmd>        Execute a openAMDC command."));
ldbLog(sdsnew("[m]axlen [len]       Trim logged openAMDC replies and Lua var dumps to len."));
...
ldbLog(sdsnew("                     next line of code."));
            ...
            ldbLog(sdsnew("<error> Unknown openAMDC Lua debugger command or "
