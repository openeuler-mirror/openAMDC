...
/* A Sentinel openAMDC Instance object is monitoring. */
...
/* This function overwrites a few normal openAMDC config default with Sentinel
 * specific defaults. */
...
void initSentinel(void) {
    ...
    /* Remove usual openAMDC commands from the command table, then just add
...
int sentinelAddrEqualsHostname(sentinelAddr *a, char *hostname) {
    ...
    if (anetResolve(NULL, hostname, ip, sizeof(ip),
         ...
         * long as the server is unavailable for some reason. It is fine since openAMDC 
 ...
 *
 * 'ri', is the openAMDC instance target of this event if applicable, and is
...
/* Create a openAMDC instance, the following fields must be populated by the
 * caller if needed:
...
/* Lookup a slave in a master openAMDC instance, by ip and port. */
...
/* This function is used in order to send commands to openAMDC instances: the
...
/* This function uses the config rewriting openAMDC engine in order to persist
 * the state of the Sentinel in the current configuration file.
 *
 * Before returning the function calls fsync() against the generated
 * configuration file to make sure changes are committed to disk.
 *
 * On failure the function logs a warning on the openAMDC log. */
void sentinelFlushConfig(void) {
    int fd = -1;
...
/* Use CLIENT SETNAME to name the connection in the openAMDC instance as
 * sentinel-<first_8_chars_of_runid>-<connection_type>
 * The connection type is "cmd" or "pubsub" as specified by 'type'.
 *
 * This makes it possible to list all the sentinel instances connected
 * to a openAMDC server with CLIENT LIST, grepping for a specific name format. */
...
void sentinelReconnectInstance(sentinelRedisInstance *ri) {
    ...
    if (link->cc == NULL) {
        ...
        if (!link->cc) {
            ...
            link->cc->data = link;
            redisAeAttach(server.el[MAIN_THREAD_ID],link->cc);
    ...
    if ((ri->flags & (SRI_MASTER|SRI_SLAVE)) && link->pc == NULL) {
        ...
        if (!link->pc) {
            ...
            link->pc->data = link;
            redisAeAttach(server.el[MAIN_THREAD_ID],link->pc);
...
/* Send a "Hello" message via Pub/Sub to the specified 'ri' openAMDC
 ...
 * sentinelSendHello() top comment for further information) to all the openAMDC
...
/* OpenAMDC instance to Redis protocol representation. */
 ...
 * when possible (that is, if the openAMDC instance is recent enough to support
...
int sentinelSendSlaveOf(sentinelRedisInstance *ri, const sentinelAddr *addr) {
    ...
    * into a master. */
     ...
     * an issue because CLIENT is variadic command, so openAMDC will not
...
int compareSlavesForPromotion(const void *a, const void *b) {
     ...
     * == NULL as there are old openAMDC versions that don't publish runid in
...
/* Perform scheduled operations for the specified openAMDC instance. */
...
void sentinelTimer(void) {
    ...
    /* We continuously change the frequency of the openAMDC "timer interrupt"
