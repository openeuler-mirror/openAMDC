...
void* activeDefragAlloc(void *ptr) {
    ...
    if(!je_get_defrag_hint(ptr)) {
        ...
        // serverLog(LL_NOTICE, "activeDefragAlloc NULL");
        return NULL;
    }
    ...
    zfree_no_tcache(ptr);
    // serverLog(LL_NOTICE, "activeDefragAlloc");
    return newptr;
}
...
void activeDefragCycle(void) {
    ...
    if (!server.active_defrag_running)
        return;
    size_t frag_b;
    float frag_f = getAllocatorFragmentation(&frag_b);
    serverLog(LL_NOTICE, "frag_pct=%f", frag_f);
