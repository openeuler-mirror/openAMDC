 ...
 * integer, because the object is going to be used as value in the openAMDC key
 ...
 * as a value in the key space, and openAMDC is configured to evict based on
 * LFU/LRU. */
...
robj *tryObjectEncoding(robj *o) {
     ...
     * everywhere in the "object space" of openAMDC and may end in places where
...
/* This implements MEMORY DOCTOR. An human readable analysis of the openAMDC
 * memory condition. */
sds getMemoryDoctorReport(void) {
    ...
    if (num_reports == 0) {
        ...
        s = sdsnew("Sam, I detected a few issues in this openAMDC instance memory implants:\n\n");
        if (big_peak) {
            s = sdscat(s," * Peak memory: In the past this instance used more than 150% the memory that is currently using. The allocator is normally not able to release memory after a peak, so you can expect to see a big fragmentation ratio, however this is actually harmless and is only due to the memory peak, and if the openAMDC instance Resident Set Size (RSS) is currently bigger than expected, the memory will be used as soon as you fill the openAMDC instance with more data. If the memory peak was only occasional and you want to try to reclaim memory, please try the MEMORY PURGE command, otherwise the only other option is to shutdown and restart the instance.\n\n");
        }
        if (high_frag) {
            s = sdscatprintf(s," * High total RSS: This instance has a memory fragmentation and RSS overhead greater than 1.4 (this means that the Resident Set Size of the openAMDC process is much larger than the sum of the logical allocations openAMDC performed). This problem is usually due either to a large peak memory (check if there is a peak memory entry above in the report) or may result from a workload that causes the allocator to fragment memory a lot. If the problem is a large peak memory, then there is no issue. Otherwise, make sure you are using the Jemalloc allocator and not the default libc malloc. Note: The currently used allocator is \"%s\".\n\n", ZMALLOC_LIB);
        }
        if (high_alloc_frag) {
        ...
        if (high_proc_rss) {
            s = sdscatprintf(s," * High process RSS overhead: This instance has non-allocator RSS memory overhead is greater than 1.1 (this means that the Resident Set Size of the openAMDC process is much larger than the RSS the allocator holds). This problem may be due to Lua scripts or Modules.\n\n");
        }
        if (big_slave_buf) {
...
int objectSetLRUOrLFU(robj *val, long long lfu_freq, long long lru_idle,
    ...
    if ((server.maxmemory_policy & MAXMEMORY_FLAG_LFU) ||
         ...
         * according to the LRU clock resolution this openAMDC
...
/* Object command allows to inspect the internals of a openAMDC Object.
 ...
 * memory introspection capabilities of openAMDC.
 *
