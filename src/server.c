 ...
 *
 * flags:       Flags as bitmask. Computed by openAMDC using the 'sflags' field.
 *
 ...
 *
 * The flags, microseconds and calls fields are computed by openAMDC and should
...
struct redisCommand redisCommandTable[] = {
    ...
    /* We don't allow PING during loading since in openAMDC PING is used as
 ...
 * function of openAMDC may be called from other threads. */
 ...
 * of view of openAMDC. Signals that are going to kill the server anyway and
 ...
 * keys and openAMDC objects as values (objects can hold SDS strings,
 * lists, sets). */
 ...
 * memory to contain hash buckets when dict expands, that may lead openAMDC
 * rejects user's requests or evicts some keys, we can stop dict to expand
 * provisionally if used memory will be over maxmemory after dict expands,
 * but to guarantee the performance of openAMDC, we still allow dict to expand
...
/* Generic hash table type where keys are openAMDC Objects, Values
 * dummy pointers. */
...
/* Db->dict, keys are sds strings, vals are openAMDC objects. */
dictType dbDictType = {
    dictSdsHash,                /* hash function */
    NULL,                       /* key dup */
    NULL,                       /* val dup */
...
/* Keylist hash table type has unencoded openAMDC objects as keys and
 ...
 * very fast: sometimes openAMDC has tens of hundreds of connected clients, and the
...
void clientsCron(int iel) {
    ...
    while(listLength(server.clients) && iterations--) {
        ...
        if (c->iel == iel) {
            ...
            * terminated. */
            ...
            if (clientsCronResizeQueryBuffer(c)) goto unlock;
            if (clientsCronTrackExpansiveClients(c, curr_peak_mem_usage_slot)) goto unlock;
    ...
    /* Process any unblocked clients to avoid potential starvation */
    if (listLength(server.unblocked_clients[threadId])) {
        processUnblockedClients(threadId);
    }

    /* Free any pending clients */
    freeClientsInAsyncFreeQueue(iel);

    /* Handle blocked clients timeout */
    handleBlockedClientsTimeout();
}

/* This function handles 'background' operations we are required to do
 * incrementally in openAMDC databases, such as active key expiring, resizing,
 * rehashing. */
void databasesCron(int iel) {
...
/*
 * Processes all clients with pending asynchronous write tasks in the specified thread.
 * It iterates over the list of clients with pending writes, handles each client's
 * asynchronous write operation, and manages the client state accordingly.
 */
void processAsyncWriteTasks() {
    /* Early return if the current thread ID is invalid */
    if (threadId == INVALID_THREAD_ID)
        return;

    serverAssert(threadOwnLock());
    
    while (listLength(server.async_pending_write_list[threadId])) {
        client *c = (client *)listNodeValue(
            listFirst(server.async_pending_write_list[threadId]));
        listDelNode(server.async_pending_write_list[threadId],
            listFirst(server.async_pending_write_list[threadId]));
...
int serverCron(struct aeEventLoop *eventLoop, long long id, void *clientData) {
     ...
     * to openAMDC. However for this to happen a given object should never be
...
next:
    ...
    databasesCron(threadId);

    /* Process asynchronous write tasks in the queue. */
     ...
     * 
     * If openAMDC is trying to failover then run the replication cron faster so
...
/* This function is designed to iterate over a list of clients associated with
 * the current thread, process their input buffers to check for complete commands,
 * and handle any asynchronous write tasks that might be pending */
void iterateAndProcessClients() {
    serverAssert(threadOwnLock());
    listIter li;
    listNode *ln;
    listRewind(server.client_handling_list[threadId], &li);
    while((listLength(server.client_handling_list[threadId]) &&
        (ln = listNext(&li)))) {
        client *c = listNodeValue(ln);
        listDelNode(server.client_handling_list[threadId], ln);
        WRAPPER_MUTEX_LOCK(cl, &c->lock);
        /* If there is more data in the client's input buffer, continue parsing it.
         * The goal is to check if there is a full command to execute. */
        processInputBuffer(c, 0, CMD_CALL_FULL);
    }

    /* Check if there are any pending asynchronous write tasks */
    if (listLength(server.async_pending_write_list[threadId])) {
        /* If there are pending write tasks, call the function to process them */
        processAsyncWriteTasks();
    }
}
...
/* This function gets called every time openAMDC is entering the
...
void beforeSleep(struct aeEventLoop *eventLoop) {
    ...
    if (ProcessingEventsWhileBlocked) {
        ...
        processed += handleClientsWithPendingWrites(threadId);
        processed += freeClientsInAsyncFreeQueue(threadId);
    ...
    /* Call the openAMDC Cluster before sleep function. Note that this function
     * may change the state of openAMDC Cluster (from ok to fail or vice versa),
     ...
     * releasing the GIL. OpenAMDC main thread will not touch anything at this
     * time. */
    server.moduleCount[threadId] = moduleCount();
    if (server.moduleCount[threadId]) moduleReleaseGIL();

    /* Do NOT add anything below moduleReleaseGIL !!! */
}

/* This function is called immediately after the event loop multiplexing
 * API returned, and the control is going to soon return to openAMDC by invoking
...
void afterSleep(struct aeEventLoop *eventLoop) {
    ...
    if (!ProcessingEventsWhileBlocked) {
        if (server.moduleCount[threadId]) moduleAcquireGIL();
    }
}
...
void createSharedObjects(void) {
    ...
    shared.loadingerr = createObject(OBJ_STRING,sdsnew(
        "-LOADING openAMDC is loading the dataset in memory\r\n"));
    shared.slowscripterr = createObject(OBJ_STRING,sdsnew(
        "-BUSY openAMDC is busy running a script. You can only call SCRIPT KILL or SHUTDOWN NOSAVE.\r\n"));
    shared.masterdownerr = createObject(OBJ_STRING,sdsnew(
        "-MASTERDOWN Link with MASTER is down and replica-serve-stale-data is set to 'no'.\r\n"));
    shared.bgsaveerr = createObject(OBJ_STRING,sdsnew(
        "-MISCONF openAMDC is configured to save RDB snapshots, but it is currently not able to persist on disk. Commands that may modify the data set are disabled, because this instance is configured to report errors during writes if RDB snapshotting fails (stop-writes-on-bgsave-error option). Please check the openAMDC logs for details about the RDB error.\r\n"));
...
void initServerConfig(void) {
     ...
     * openamdc.conf using the rename-command directive. */
     ...
     * Redis 5. However it is possible to revert it via openamdc.conf. */
...
int restartServer(int flags, mstime_t delay) {
     ...
     * which are useful if we restart a openAMDC server which is not daemonized. */
...
void closeSocketListeners(socketFds *sfd) {
    ...
    for (j = 0; j < sfd->count; j++) {
        ...
        aeDeleteFileEvent(server.el[threadId], sfd->fd[j], AE_READABLE);
        close(sfd->fd[j]);
    }
...
int createSocketAcceptHandler(socketFds *sfd, aeFileProc *accept_handler) {
    ...
    for (j = 0; j < sfd->count; j++) {
        if (aeCreateFileEvent(server.el[threadId], sfd->fd[j], AE_READABLE, accept_handler,NULL) == AE_ERR) {
            /* Rollback */
            for (j = j-1; j >= 0; j--) aeDeleteFileEvent(server.el[threadId], sfd->fd[j], AE_READABLE);
            return C_ERR;
        }
    }
    return C_OK;
 ...
 * binding the addresses specified in the openAMDC server configuration.
 *
...
int listenToPort(int port, socketFds *sfd, int reusePort) {
    int j;
    ...
    for (j = 0; j < bindaddr_count; j++) {
        ...
        if (strchr(addr,':')) {
            /* Bind IPv6 address. */
            sfd->fd[sfd->count] = anetTcp6Server(server.neterr[threadId],port,addr,server.tcp_backlog, reusePort);
        } else {
            /* Bind IPv4 address. */
            sfd->fd[sfd->count] = anetTcpServer(server.neterr[threadId],port,addr,server.tcp_backlog, reusePort);
        }
        if (sfd->fd[sfd->count] == ANET_ERR) {
            int net_errno = errno;
            serverLog(LL_WARNING,
                "Warning: Could not create server TCP listening socket %s:%d: %s",
                addr, port, server.neterr[threadId]);
...
static void initNetworkingForThread(int iel) {
    /* Open the TCP listening socket for the user commands. */
    if (server.worker_threads_num > 1 || (iel == MAIN_THREAD_ID)) {
        if (server.port != 0 &&
            listenToPort(server.port,&server.ipfd[iel], server.worker_threads_num > 1) == C_ERR) {
            serverLog(LL_WARNING, "Failed listening on port %u (TCP), aborting.", server.port);
            exit(1);
        }
        if (server.tls_port != 0 &&
            listenToPort(server.tls_port,&server.tlsfd[iel], server.worker_threads_num > 1) == C_ERR) {
            serverLog(LL_WARNING, "Failed listening on port %u (TLS), aborting.", server.port);
            exit(1);
        }
    } else {
        // We use the main threads file descriptors
        memcpy(&server.ipfd[iel], &server.ipfd[MAIN_THREAD_ID], sizeof(socketFds));
        server.ipfd[iel].count = server.ipfd[MAIN_THREAD_ID].count;
    }

    /* Create an event handler for accepting new connections in TCP */
    for (int j = 0; j < server.ipfd[iel].count; j++) {
        if (aeCreateFileEvent(server.el[iel], server.ipfd[iel].fd[j], AE_READABLE|AE_READ_THREADSAFE,
            acceptTcpHandler,NULL) == AE_ERR) {
                serverPanic(
                    "Unrecoverable error creating server.ipfd[%d] file event.", iel);
            }
    }

    makeThreadKillable();

    for (int j = 0; j < server.tlsfd[iel].count; j++) {
        if (aeCreateFileEvent(server.el[iel], server.tlsfd[iel].fd[j], AE_READABLE|AE_READ_THREADSAFE,
            acceptTLSHandler,NULL) == AE_ERR) {
                serverPanic(
                    "Unrecoverable error creating server.tlsfd[%d] file event.", iel);
            }
    }
}
...
void initServer(void) {
    ...
    /* Create the openAMDC databases, and initialize other internal state. */
    for (j = 0; j < server.dbnum; j++) {
    ...
    for (int iel = 0; iel < MAX_THREAD_VAR; iel++) {
        if (iel < server.worker_threads_num || 
            ...
            server.async_pending_write_list[iel] = listCreate();
            server.clients_timeout_table[iel] = raxNew();
            server.fixed_time_expire[iel] = 0;
            server.ipfd[iel].count = 0;
            server.tlsfd[iel].count = 0;
            server.in_eval[iel] = 0;
            server.in_exec[iel] = 0;
            server.moduleCount[iel] = 0;
            server.clients_num_in_thread[iel] = 0;
            server.stat_total_error_replies[iel] = 0;
            server.in_nested_call[iel] = 0;
            server.propagate_in_transaction[iel] = 0;
            server.client_pause_in_transaction[iel] = 0;
            server.current_client[iel] = NULL;
            server.lua_client[iel] = NULL;
            server.el[iel] = aeCreateEventLoop(server.maxclients+CONFIG_FDSET_INCR);
            if (server.el[iel] == NULL) {
                serverLog(LL_WARNING, "Failed creating the event loop, %s", strerror(errno));
                exit(1);
            }
            ...
            if (aeCreateTimeEvent(server.el[iel], 1, serverCron, NULL, NULL) == AE_ERR) {
            ...
            * when a blocked client in a module needs attention. */
            if (aeCreateFileEvent(server.el[iel], server.module_blocked_pipe[0], AE_READABLE,
    ...
    initNetworkingForThread(MAIN_THREAD_ID);
    threadId = MAIN_THREAD_ID;

    /* Open the listening Unix domain socket. */
    if (server.unixsocket != NULL) {
        unlink(server.unixsocket); /* don't care if this fails */
        server.sofd = anetUnixServer(server.neterr[threadId], server.unixsocket,
            server.unixsocketperm, server.tcp_backlog);
        if (server.sofd == ANET_ERR) {
            serverLog(LL_WARNING, "Opening Unix socket: %s", server.neterr[threadId]);
            exit(1);
        }
    ...
    if (server.ipfd[MAIN_THREAD_ID].count == 0 &&
        server.tlsfd[MAIN_THREAD_ID].count == 0 &&
        server.sofd < 0) {
    ...
    if (server.sofd > 0 &&
        aeCreateFileEvent(server.el[MAIN_THREAD_ID],server.sofd,AE_READABLE|AE_READ_THREADSAFE,
        acceptUnixHandler,NULL) == AE_ERR) serverPanic("Unrecoverable error creating server.sofd[0] file event.");
     ...
     * useless crashes of the openAMDC instance for out of memory. */
...
/* Populates the openAMDC Command Table starting from the hard coded list
...
void populateCommandTable(void) {
    ...
    for (j = 0; j < numcommands; j++) {
         ...
         * by rename-command statements in openamdc.conf. */
 ...
 * openamdc.conf rename-command statement.
 *
               ...
               int flags)
{
    ...
    if (server.in_exec[threadId] && !server.propagate_in_transaction[threadId])
        execCommandPropagateMulti(dbid);

    /* This needs to be unreachable since the dataset should be fixed during 
     * client pause, otherwise data may be lossed during a failover. */
    serverAssert(!(areClientsPaused() && !server.client_pause_in_transaction[threadId]));
 ...
 *
 * 'cmd' must be a pointer to the openAMDC command to replicate, dbid is the
 * database ID the command should be propagated into.
 * Arguments of the command to propagate are passed as an array of openAMDC
 ...
 * openAMDC command implementation in order to to force the propagation of a
...
/* Call() is the core of openAMDC execution of a command.
 *
...
void call(client *c, int flags) {
    ...
    /* Call the command. */
    dirty = server.dirty;
    prev_err_count[threadId] = server.stat_total_error_replies[threadId];
    ...
    server.in_nested_call[threadId]++;
    c->cmd->proc(c);
    server.in_nested_call[threadId]--;
    ...
    if ((server.stat_total_error_replies[threadId] - prev_err_count[threadId]) > 0) {
    ...
    if (!server.in_exec[threadId] && server.client_pause_in_transaction[threadId]) {
        server.client_pause_in_transaction[threadId] = 0;
    }
    ...
    server.fixed_time_expire[threadId]--;
...
void afterCommand(client *c) {
    ...
    if (!server.in_nested_call[threadId]) trackingHandlePendingKeyInvalidations();
}
...
int processCommand(client *c, int callFlags) {
    ...
    if (!server.lua_timedout) {
         ...
         * some commands. */
        serverAssert(!server.propagate_in_transaction[threadId]);
        serverAssert(!server.in_exec[threadId]);
        serverAssert(!server.in_eval[threadId]);
    }
    ...
    if (server.maxmemory && !server.lua_timedout) {
        ...
        if (server.current_client[threadId] == NULL) return C_ERR;
...
void closeListeningSockets(int unlink_unix_socket) {
    int j;
    for (int iel = 0; iel < server.worker_threads_num; iel++) {
        for (j = 0; j < server.ipfd[iel].count; j++) close(server.ipfd[iel].fd[j]);
        for (j = 0; j < server.tlsfd[iel].count; j++) close(server.tlsfd[iel].fd[j]);
    }
...
/* Asynchronously signals an thread to stop processing events */
void asyncAeEventStop(void *var, aeAsyncCallback *callback) {
    UNUSED(callback);
    int iel = (int)((int64_t)var);
    server.el[iel]->stop = 1;
}
...
int prepareForShutdown(int flags) {
    ...
    if (server.child_type == CHILD_TYPE_RDB) {
         ...
         * The temp rdb file fd may won't be closed when openAMDC exits quickly,
    ...
    if ((server.saveparamslen > 0 && !nosave) || save) {
        ...
        if (rdbSave(server.rdb_filename,rsiptr) != C_OK) {
             ...
             * in the next cron() openAMDC will be notified that the background
    ...
    /* For each valid thread or a specific module thread, it creates a void
     * pointer var pointing to the current thread index and asynchronously
    ...
    serverLog(LL_WARNING,"%s is now ready to exit, bye bye...",
        server.sentinel_mode ? "sentinel" : "openAMDC");
    return C_OK;
}

/*================================== Commands =============================== */

/* Sometimes openAMDC cannot accept write commands because there is a persistence
 * error with the RDB or AOF file, and openAMDC is configured in order to stop
...
/* Output the representation of a openAMDC command. Used by the COMMAND command. */
...
void commandCommand(client *c) {
    ...
    if (c->argc == 2 && !strcasecmp(c->argv[1]->ptr,"help")) {
        const char *help[] = {
"(no subcommand)",
"    Return details about all openAMDC commands.",
"COUNT",
"    Return the total number of commands in this openAMDC server.",
"GETKEYS <full-command>",
"    Return the keys from a full openAMDC command.",
"INFO [<command-name> ...]",
"    Return details about multiple openAMDC commands.",
NULL
        };
        addReplyHelp(c, help);
    } else if (c->argc == 1) {
...
sds genRedisInfoString(const char *section) {
    ...
    if (allsections || defsections || !strcasecmp(section,"clients")) {
        ...
        for (int iel = 0; iel < server.worker_threads_num; iel++) {
            info = sdscatprintf(info, "thread_%d_clients_in_timeout_table:%llu\r\n", iel,
                (unsigned long long)raxSize(server.clients_timeout_table[iel]));
        }
        for (int iel = 0; iel < server.worker_threads_num; iel++) {
            info = sdscatprintf(info, "thread_%d_clients:%d\r\n", iel, server.clients_num_in_thread[iel]);
        }
    }
    ...
    if (allsections || defsections || !strcasecmp(section,"stats")) {
        ...
        long long stat_net_input_bytes, stat_net_output_bytes, stat_total_error_replies = 0;
        atomicGet(server.stat_total_reads_processed, stat_total_reads_processed);
        atomicGet(server.stat_total_writes_processed, stat_total_writes_processed);
        atomicGet(server.stat_net_input_bytes, stat_net_input_bytes);
        atomicGet(server.stat_net_output_bytes, stat_net_output_bytes);
        for (int iel = 0; iel < server.worker_threads_num; ++iel)
            stat_total_error_replies += server.stat_total_error_replies[iel];
...
void linuxMemoryWarnings(void) {
    ...
    if (THPIsEnabled() && THPDisable()) {
        serverLog(LL_WARNING,"WARNING you have Transparent Huge Pages (THP) support enabled in your kernel. This will create latency and memory usage issues with openAMDC. To fix this issue run the command 'echo madvise > /sys/kernel/mm/transparent_hugepage/enabled' as root, and add it to your /etc/rc.local in order to retain the setting after a reboot. openAMDC must be restarted after THP is disabled (set to 'madvise' or 'never').");
    }
}
...
void daemonize(void) {
    ...
    /* Every output goes to /dev/null. If openAMDC is daemonized but
...
void version(void) {
    printf("openAMDC server v=%s sha=%s:%d malloc=%s bits=%d build=%llx\n",
        OPENAMDC_VERSION,
        redisGitSHA1(),
...
void usage(void) {
    fprintf(stderr,"Usage: ./openamdc-server [/path/to/openamdc.conf] [options] [-]\n");
    fprintf(stderr,"       ./openamdc-server - (read config from stdin)\n");
    fprintf(stderr,"       ./openamdc-server -v or --version\n");
    fprintf(stderr,"       ./openamdc-server -h or --help\n");
    fprintf(stderr,"       ./openamdc-server --test-memory <megabytes>\n\n");
    fprintf(stderr,"Examples:\n");
    fprintf(stderr,"       ./openamdc-server (run the server with default conf)\n");
    fprintf(stderr,"       ./openamdc-server /etc/openamdc/6379.conf\n");
    fprintf(stderr,"       ./openamdc-server --port 7777\n");
    fprintf(stderr,"       ./openamdc-server --port 7777 --replicaof 127.0.0.1 8888\n");
    fprintf(stderr,"       ./openamdc-server /etc/myredis.conf --loglevel verbose -\n");
    fprintf(stderr,"       ./openamdc-server /etc/myredis.conf --loglevel verbose\n\n");
    fprintf(stderr,"Sentinel mode:\n");
    fprintf(stderr,"       ./openamdc-server /etc/sentinel.conf --sentinel\n");
    exit(1);
}
...
#include "asciilogo.h"
     ...
     * forced us to do so via openamdc.conf. */
...
int changeBindAddr(sds *addrlist, int addrlist_len) {
    ...
    closeSocketListeners(&server.ipfd[threadId]);
    closeSocketListeners(&server.tlsfd[threadId]);
    ...
    if ((server.port != 0 && listenToPort(server.port, &server.ipfd[threadId], server.worker_threads_num > 1) != C_OK) ||
        (server.tls_port != 0 && listenToPort(server.tls_port, &server.tlsfd[threadId], server.worker_threads_num > 1) != C_OK)) {
        ...
        server.ipfd[threadId].count = 0;
        if (server.port != 0 && listenToPort(server.port, &server.ipfd[threadId], server.worker_threads_num > 1) != C_OK) {
            serverPanic("Failed to restore old listening sockets.");
        }

        server.tlsfd[threadId].count = 0;
        if (server.tls_port != 0 && listenToPort(server.tls_port, &server.tlsfd[threadId], server.worker_threads_num > 1) != C_OK) {
            serverPanic("Failed to restore old listening sockets.");
        }
    ...
    if (createSocketAcceptHandler(&server.ipfd[threadId], acceptTcpHandler) != C_OK) {
        serverPanic("Unrecoverable error creating TCP socket accept handler.");
    }
    if (createSocketAcceptHandler(&server.tlsfd[threadId], acceptTLSHandler) != C_OK) {
...
int changeListenPort(int port, socketFds *sfd, aeFileProc *accept_handler) {
    ...
    /* Bind to the new port */
    if (listenToPort(port, &new_sfd, server.worker_threads_num > 1) != C_OK) {
        return C_ERR;
    }
 ...
 * argv[0] contains "openamdc-sentinel". */
int checkForSentinelMode(int argc, char **argv) {
    int j;

    if (strstr(argv[0],"openamdc-sentinel") != NULL) return 1;
    for (j = 1; j < argc; j++)
...
void redisOutOfMemoryHandler(size_t allocation_size) {
    serverLog(LL_WARNING,"Out Of Memory allocating %zu bytes!",
        allocation_size);
    serverPanic("openAMDC aborting for OUT OF MEMORY. Allocating %zu bytes!",
        allocation_size);
}

/* Callback for sdstemplate on proc-title-template. See openamdc.conf for
 * supported variables.
 */
...
static int redisSupervisedSystemd(void) {
    ...
    serverLog(LL_WARNING,
            "systemd supervision requested or auto-detected, but openAMDC is compiled without libsystemd support!");
    return 0;
#else
    if (redisCommunicateSystemd("STATUS=openAMDC is loading...\n") <= 0)
        return 0;
    serverLog(LL_NOTICE,
...
int main(int argc, char **argv) {
    ...
    if (argc >= 2) {
        ...
        if (strcmp(argv[1], "--test-memory") == 0) {
            if (argc == 3) {
                ...
                fprintf(stderr,"Example: ./openamdc-server --test-memory 4096\n\n");
                exit(1);
            }
        }
    ...
    serverLog(LL_WARNING, "oO0OoO0OoO0Oo openAMDC is starting oO0OoO0OoO0Oo");
    serverLog(LL_WARNING,
        "openAMDC version=%s, bits=%d, commit=%s, modified=%d, pid=%d, just started",
            OPENAMDC_VERSION,
            (sizeof(long) == 8) ? 64 : 32,
            redisGitSHA1(),
            strtol(redisGitDirty(),NULL,10) > 0,
    ...
    if (argc == 1) {
        serverLog(LL_WARNING, "Warning: no config file specified, using the default config. In order to specify a config file use %s /path/to/openamdc.conf", argv[0]);
    } else {
    ...
    if (!server.sentinel_mode) {
        ...
        if ((ret = linuxMadvFreeForkBugCheck())) {
            ...
            if (!checkIgnoreWarning("ARM64-COW-BUG")) {
                serverLog(LL_WARNING,"openAMDC will now exit to prevent data corruption. "
