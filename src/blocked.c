 ...
 * client->bpop.keys hash table. The keys of the hash table are openAMDC
...
void processUnblockedClients(int iel) {
    ...
    while (listLength(server.unblocked_clients[iel])) {
        ln = listFirst(server.unblocked_clients[iel]);
        serverAssert(ln != NULL);
        c = ln->value;
        listDelNode(server.unblocked_clients[iel],ln);
...
void queueClientForReprocessing(client *c) {
    ...
    if (!(c->flags & CLIENT_UNBLOCKED)) {
        c->flags |= CLIENT_UNBLOCKED;
        listAddNodeTail(server.unblocked_clients[c->iel],c);
    }
}
...
void serveClientsBlockedOnListKey(robj *o, readyList *rl) {
    ...
    if (de) {
        ...
        while(numclients--) {
            ...
            WRAPPER_MUTEX_LOCK(cl, &receiver->lock);
            ...
            if (value) {
                ...
                server.current_client[threadId] = receiver;
                monotime replyTimer;
...
/* This function should be called by openAMDC every time a single command,
 ...
 * be used only for a single type, like virtually any openAMDC application will
