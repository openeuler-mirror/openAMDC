...
void aofChildWriteDiffData(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    serverAssert(el == server.el[MAIN_THREAD_ID]);
    listNode *ln;
    aofrwblock *block;
    ssize_t nwritten;
    UNUSED(fd);
...
/* Schedules an asynchronous write of AOF diff data to the child process. */
void asyncAofChildWriteDiffData(void *var, aeAsyncCallback *callback) {
    UNUSED(var);
    UNUSED(callback);
    server.aof_child_write_diff_installed = 1;
    if (!server.aof_stop_sending_diff &&
        aeGetFileEvents(server.el[MAIN_THREAD_ID],server.aof_pipe_write_data_to_child) == 0)
    {
        aeCreateFileEvent(server.el[MAIN_THREAD_ID], server.aof_pipe_write_data_to_child,
...
void aofRewriteBufferAppend(unsigned char *s, unsigned long len) {
    ...
    if (server.aof_child_write_diff_installed) {
        aeAsyncFunction(server.el[MAIN_THREAD_ID], asyncAofChildWriteDiffData, NULL, NULL, 1);
    }
}
...
int startAppendOnly(void) {
    ...
    if (newfd == -1) {
        ...
        serverLog(LL_WARNING,
            "openAMDC needs to enable the AOF but can't open the "
    ...
    if (hasActiveChildProcess() && server.child_type != CHILD_TYPE_AOF) {
        ...
        if (rewriteAppendOnlyFileBackground() == C_ERR) {
            close(newfd);
            serverLog(LL_WARNING,"openAMDC needs to enable the AOF but can't trigger a background AOF rewrite operation. Check the above logs for more info about the error.");
            return C_ERR;
        }
    }
...
void flushAppendOnlyFile(int force) {
    ...
    if (server.aof_fsync == AOF_FSYNC_EVERYSEC && !force) {
        ...
        if (sync_in_progress) {
            ...
            serverLog(LL_NOTICE,"Asynchronous AOF fsync is taking too long (disk is busy?). Writing the AOF buffer without waiting for fsync to complete, this may slow down openAMDC.");
        }
    }
    ...
    if (nwritten != (ssize_t)sdslen(server.aof_buf)) {
            ...
            if (ftruncate(server.aof_fd, server.aof_current_size) == -1) {
                if (can_log) {
                    serverLog(LL_WARNING, "Could not remove short write "
                             "from the append-only file.  openAMDC may refuse "
        ...
        if (server.aof_last_write_status == C_ERR) {
            serverLog(LL_WARNING,
                "AOF write error looks solved, openAMDC can write again.");
            server.aof_last_write_status = C_OK;
        }
    }
    server.aof_current_size += nwritten;
...
/* In openAMDC commands are always executed in the context of a client, so in
...
struct client *createAOFClient(void) {
     ...
     * so that openAMDC will not try to send replies to this client. */
...
uxeof: /* Unexpected AOF end of file. */
    ...
    if (fakeClient) freeFakeClient(fakeClient); /* avoid valgrind warning */
    fclose(fp);
    serverLog(LL_WARNING,"Unexpected end of file reading the append only file. You can: 1) Make a backup of your AOF file, then use ./openAMDC-check-aof --fix <filename>. 2) Alternatively you can set the 'aof-load-truncated' configuration option to yes and restart the server.");
    exit(1);

fmterr: /* Format error. */
    if (fakeClient) freeFakeClient(fakeClient); /* avoid valgrind warning */
    fclose(fp);
    serverLog(LL_WARNING,"Bad file format reading the append only file: make a backup of your AOF file, then use ./openAMDC-check-aof --fix <filename>");
    exit(1);
}
 ...
 * The 'hi' argument passes a valid openAMDC hash iterator.
 ...
 * that is exported by a module and is not handled by openAMDC itself.
 * The function returns 0 on error, 1 on success. */
 ...
 * log openAMDC uses variadic commands when possible, such as RPUSH, SADD
...
int rewriteAppendOnlyFile(char *filename) {
    ...
    server.aof_child_write_diff_installed = 0;
...
void aofChildPipeReadable(aeEventLoop *el, int fd, void *privdata, int mask) {
     ...
     * rewrite. */
    aeDeleteFileEvent(el,server.aof_pipe_read_ack_from_child,AE_READABLE);
}
...
int aofCreatePipes(void) {
    ...
    if (aeCreateFileEvent(server.el[threadId], fds[2], AE_READABLE, aofChildPipeReadable, NULL) == AE_ERR) goto error;
    ...
    server.aof_pipe_read_ack_from_child_el = server.el[threadId];
    return C_OK;
...
/* Asynchronously deletes a file event for readability and closes the
 * associated file descriptor. */
void asyncDeleteFileEventReadable(void *var, aeAsyncCallback *callback) {
    UNUSED(callback);
    int fd = (int)((int64_t)var);
    aeDeleteFileEvent(server.el[threadId], fd, AE_READABLE);
    close(fd);
}

/* Asynchronously deletes a file event for writability and closes the
 * associated file descriptor. */
void asyncDeleteFileEventWritable(void *var, aeAsyncCallback *callback) {
    UNUSED(callback);
    int fd = (int)((int64_t)var);
    aeDeleteFileEvent(server.el[threadId], fd, AE_WRITABLE);
    close(fd);
}

/* Closes all AOF interprocess communication pipes and removes associated
 * file events. */
void aofClosePipes(void) {
    void *var = (void*)((int64_t)server.aof_pipe_read_ack_from_child);
    serverAssert(aeAsyncFunction(server.aof_pipe_read_ack_from_child_el, asyncDeleteFileEventReadable, var, NULL, 1) == AE_OK);

    var = (void*)((int64_t)server.aof_pipe_write_data_to_child);
    serverAssert(aeAsyncFunction(server.el[MAIN_THREAD_ID], asyncDeleteFileEventWritable, var, NULL, 1) == AE_OK);
 ...
 * 2) openAMDC calls this function, that forks():
...
int rewriteAppendOnlyFileBackground(void) {
    ...
    if ((childpid = redisFork(CHILD_TYPE_AOF)) == 0) {
        ...
        /* Child */
        redisSetProcTitle("openAMDC-aof-rewrite");
