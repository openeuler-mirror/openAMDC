...
void resizeReplicationBacklog(long long newsize) {
    ...
    if (server.repl_backlog != NULL) {
         ...
         * old buffer. */
        WRAPPER_MUTEX_LOCK(rl, &replBacklogLock);
...
/* Wrapper for feedReplicationBacklog() that takes openAMDC string objects
 * as input. */
...
void replicationFeedSlaves(list *slaves, int dictid, robj **argv, int argc) {
    ...
    if (server.slaveseldb != dictid) {
        ...
        while((ln = listNext(&li))) {
            ...
            if (slave->flags & CLIENT_CLOSE_ASAP) continue;
            WRAPPER_MUTEX_LOCK(sl, &slave->lock);
...
void replicationFeedMonitors(client *c, list *monitors, int dictid, robj **argv, int argc) {
    ...
    while((ln = listNext(&li))) {
        ...
        WRAPPER_MUTEX_DEFER_LOCK(wm, &monitor->lock);
...
void syncCommand(client *c) {
    ...
    if (!strcasecmp(c->argv[0]->ptr,"psync")) {
         ...
         * of the replication protocol (like openamdc-cli --slave). Flag the client
 ...
 * - listening-port <port>
 * - ip-address <ip>
 * What is the listening ip and port of the Replica openAMDC instance, so that
...
void putSlaveOnline(client *slave) {
    ...
    if (connSetWriteHandler(slave->conn, sendReplyToClient, 1) == C_ERR) {
...
void sendBulkToSlave(connection *conn) {
    ...
    serverAssert(clientCheckThreadAligned(slave));
    WRAPPER_MUTEX_LOCK(sl, &slave->lock);
    ...
    if (slave->repldboff == slave->repldbsize) {
        ...
        slave->repldbfd = -1;
        connSetWriteHandler(slave->conn,NULL,0);
        putSlaveOnline(slave);
    }
}

void asyncCreateFileEvent(void *conn, aeAsyncCallback *callback) {
    UNUSED(callback);
    client *slave = connGetPrivateData(conn);
    WRAPPER_MUTEX_LOCK(sl, &slave->lock);
    if (aeCreateFileEvent(server.el[threadId], server.rdb_pipe_read, AE_READABLE, rdbPipeReadHandler, NULL) == AE_ERR) {
...
void rdbPipeWriteHandlerConnRemoved(struct connection *conn) {
    if (!connHasWriteHandler(conn))
        return;
    connSetWriteHandler(conn, NULL, 0);
    client *slave = connGetPrivateData(conn);
    slave->repl_last_partial_write = 0;
    server.rdb_pipe_numconns_writing--;
    /* if there are no more writes for now for this conn, or write error: */
    if (server.rdb_pipe_numconns_writing == 0) {
        aeAsyncFunction(server.el[MAIN_THREAD_ID], asyncCreateFileEvent, conn, NULL, 1);
    }
}
...
void asyncSetWriteHandler(void *var) {
    client *slave = var;
    connSetWriteHandler(slave->conn, rdbPipeWriteHandler, 0);
}
...
void rdbPipeReadHandler(struct aeEventLoop *eventLoop, int fd, void *clientData, int mask) {
    ...
    serverAssert(eventLoop == server.el[MAIN_THREAD_ID]);
    ...
    while (1) {
        ...
        if (server.rdb_pipe_bufflen == 0) {
            ...
            int stillUp = 0;
            aeDeleteFileEvent(eventLoop, server.rdb_pipe_read, AE_READABLE);
            for (i=0; i < server.rdb_pipe_numconns; i++)
            {
                connection *conn = server.rdb_pipe_conns[i];
                if (!conn)
        ...
        for (i=0; i < server.rdb_pipe_numconns; i++)
        {
            ...
            if (nwritten != server.rdb_pipe_bufflen) {
                slave->repl_last_partial_write = server.unixtime;
                server.rdb_pipe_numconns_writing++;
                clientAsyncFuntion(slave, asyncSetWriteHandler, 1);
            }
            stillAlive++;
        }
...
void asyncConnSetWriteHandler(void *var) {
    client *slave = var;
    connSetWriteHandler(slave->conn, NULL, 0);
    if (connSetWriteHandler(slave->conn, sendBulkToSlave, 0) == C_ERR) {
        freeClient(slave);
    }
}
    ...
    while((ln = listNext(&li))) {
        ...
        if (slave->replstate == SLAVE_STATE_WAIT_BGSAVE_END) {
            ...
            if (type == RDB_CHILD_TYPE_SOCKET) {
                ...
                if (clientCheckThreadAligned(slave)) {
                    connSetWriteHandler(slave->conn,NULL,0);
                    if (connSetWriteHandler(slave->conn,sendBulkToSlave,0) == C_ERR) {
                        wrapperMutexUnlock(&sl);
                        freeClientAsync(slave);
                    ...
                    clientAsyncFuntion(slave, asyncConnSetWriteHandler, 1);
                }               
            }
        }
    }
...
void replicationCreateMasterClient(connection *conn, int dbid) {
    server.master = createClient(conn, threadId);
    if (conn)
        connSetReadHandler(server.master->conn, readQueryFromClient, 1);
 ...
 * database loading is used, openAMDC makes a backup of the existing databases
...
void readSyncBulkPayload(connection *conn) {
     ...
     *    directly from the socket to the openAMDC memory, without using
    ...
    connSetReadHandler(conn, NULL, 0);
...
char *sendCommand(connection *conn, ...) {
    ...
    /* Create the command to send to the master, we use openAMDC binary
...
void syncWithMaster(connection *conn) {
    ...
    if (server.repl_state == REPL_STATE_CONNECTING) {
        ...
        connSetReadHandler(conn, syncWithMaster, 0);
        connSetWriteHandler(conn, NULL, 0);
    ...
    if (server.repl_state == REPL_STATE_RECEIVE_PING_REPLY) {
         ...
         * Note that older versions of openAMDC replied with "operation not
    ...
    if (server.repl_state == REPL_STATE_RECEIVE_PORT_REPLY) {
        err = receiveSynchronousResponse(conn);
        /* Ignore the error if any, not all the openAMDC versions support
    ...
    if (connSetReadHandler(conn, readSyncBulkPayload, 0) == C_ERR)
    {
...
int connectWithMaster(void) {
    ...
    server.repl_transfer_iel = threadId;
...
/* Asynchronously close an connection */
void asyncConnClose(void *var, aeAsyncCallback *callback) {
    UNUSED(callback);
    connection *conn = var;
    connClose(conn);
}
...
void undoConnectWithMaster(void) {
    serverAssert(threadOwnLock());
    connection *conn = server.repl_transfer_s;
    server.repl_transfer_s = NULL;
    int result = aeAsyncFunction(server.el[server.repl_transfer_iel],
        asyncConnClose, conn, NULL, 0);
...
/* Disconnects the master client connection, handling various scenarios based
 * on the client state */
void disconnectMaster(client **master) {
    serverAssert(master != NULL);
    client *m = *master;
    
    /* Early exit if master is already NULL */
    if (m == NULL) return;
    
    /* If the client is in the correct thread, directly free it */
    if (clientCheckThreadAligned(m)) {
        freeClient(m);
        return;
    }

    /* Handle clients with protocol errors or blocked status */
    if (m->flags & (CLIENT_PROTOCOL_ERROR|CLIENT_BLOCKED)) {
        freeClientAsync(m);
        *master = NULL;
        return;
    }

    /* Create a new client that will be the master */
    client *c = createClient(NULL, threadId);
    c->flags |= m->flags & ~(CLIENT_PENDING_WRITE | CLIENT_UNBLOCKED | CLIENT_CLOSE_ASAP);
    c->authenticated = m->authenticated;
    c->reploff = m->reploff;
    c->read_reploff = m->read_reploff;
    c->user = m->user;
    c->replstate = m->replstate;
    c->psync_initial_offset = m->psync_initial_offset;
    c->repldboff = m->repldboff;
    c->repldbsize = m->repldbsize;
    memcpy(c->replid, m->replid, sizeof(m->replid));
    selectDb(c, m->db->id);

    /* Clear the CLIENT_MASTER flag from the old master and free it asynchronously */
    m->flags &= ~CLIENT_MASTER;
    freeClientAsync(m);

    /* Assign the new client as the master and update replication cache */
    *master = c;
...
void replicationCacheMaster(client *c) {
    ...
    if (listLength(server.master->parsed_list))
        listEmpty(server.master->parsed_list);
    server.master->multibulklen = 0;
    server.master->read_reploff = server.master->reploff;
...
void replicationCacheMasterUsingMyself(void) {
    ...
    WRAPPER_MUTEX_LOCK(ml, &server.master->lock);
...
void replicationResurrectCachedMaster(connection *conn) {
    ...
    serverAssert(clientCheckThreadAligned(server.master));
    if (connSetReadHandler(server.master->conn, readQueryFromClient, 1)) {
    ...
    if (clientHasPendingReplies(server.master)) {
        if (connSetWriteHandler(server.master->conn, sendReplyToClient, 1)) {
 ...
 * OpenAMDC synchronous replication design can be summarized in points:
 *
 * - OpenAMDC masters have a global replication offset, used by PSYNC.
