...
"    unit tests to simulate openAMDC errors.",
"LOG <message>",
"    Write <message> to the server log.",
"HTSTATS <dbid>",
"    Return hash table statistics of the specified openAMDC database.",
"HTSTATS-KEY <key>",
...
"    accessed (otherwise the openAMDC behavior). Setting it to 1 reenables back the",
"    default.",
...
"STRUCTSIZE",
"    Return the size of different openAMDC core C structures.",
"ZIPLIST <key>",
...
NULL
    ...
    } else if (!strcasecmp(c->argv[1]->ptr,"stringmatch-test") && c->argc == 2)
    {
        ...
        addReplyStatus(c,"Apparently openAMDC did not crash: test passed");
...
void bugReportStart(void) {
    ...
    if (bug_report_start == 0) {
        ...
        "\n\n=== openAMDC BUG REPORT START: Cut & paste starting from here ===\n");
        bug_report_start = 1;
    }
...
/* Return a file descriptor to write directly to the openAMDC log with the
 * write(2) syscall, that can be used in critical sections of the code
 * where the rest of openAMDC can't be trusted (for example during the memory
 ...
 * currently being served by openAMDC. May be NULL if openAMDC is not serving a
 * client right now. */
void logCurrentClient(void) {
    if (server.current_client[threadId] == NULL) return;

    client *cc = server.current_client[threadId];
    sds client;
    int j;
 ...
 * Currently openAMDC does this only on crash (for instance on SIGSEGV) in order
 * to perform a fast memory check without other threads messing with memory. */
void killThreads(void) {
    killMainAndWorkerThreads();
    bioKillThreads();
}
...
void sigsegvHandler(int sig, siginfo_t *info, void *secret) {
    ...
    bugReportStart();
    serverLog(LL_WARNING,
        "openAMDC %s crashed by signal: %d, si_code: %d", OPENAMDC_VERSION, sig, info->si_code);
...
void bugReportEnd(int killViaSignal, int sig) {
    ...
    serverLogRaw(LL_WARNING|LL_RAW,
"\n=== openAMDC BUG REPORT END. Make sure to include from START to END. ===\n\n"
"       Please report the crash by opening an issue on gitee:\n\n"
"           http://gitee.com/openeuler/openAMDC/issues\n\n"
"  If a openAMDC module was involved, please open in the module's repo instead.\n\n"
"  Suspect RAM error? Use openAMDC-server --test-memory to verify it.\n\n"
"  Some other issues could be detected by openAMDC-server --check-system\n"
);
