...
#include "redismodule.h"    /* OpenAMDC modules API defines. */

/* Following includes allow test functions to be called from openAMDC main() */
#include "zipmap.h"
#include "sha1.h"
...
#define CONFIG_DEFAULT_PID_FILE "/var/run/openamdc.pid"
...
#define MAX_THREAD_VAR (MAX_THREADS+1)
#define INVALID_THREAD_ID -1
#define MAIN_THREAD_ID 0
#define MODULE_THREAD_ID MAX_THREADS
...
#define CMD_MODULE_NO_CLUSTER (1ULL<<18) /* Deny on openAMDC Cluster. */
...
#define CMD_CATEGORY_REPLICATION (1ULL << 40)
...
#define CLIENT_INPROGRESS_COMMAND (1<<29) /* Ensure we don't process a client already executing */
...
/* OpenAMDC maxmemory strategies. Instead of using just incremental number
...
/* A openAMDC object, that is a type able to hold a string / list / set */

/* The actual openAMDC Object */
 ...
 * is one directly managed by a openAMDC module. In this case the value points
...
/* In openAMDC objects 'robj' structures of type OBJ_MODULE, the value pointer
...
/* This is a wrapper for the 'rio' streams used inside rdb.c in openAMDC, so that
...
/* This is a structure used to export DEBUG DIGEST capabilities to openAMDC
...
/* Macro used to initialize a openAMDC object allocated on the stack.
...
/* OpenAMDC database representation. There are multiple databases identified
...
typedef struct blockingState {
    ...
    void *module_blocked_handle; /* RedisModuleBlockedClient structure.
                                    which is opaque for the openAMDC core, only
                                    handled in module.c. */
} blockingState;
 ...
 * also called ready_keys in every structure representing a openAMDC database,
...
/* This structure represents a openAMDC user. This is useful for ACLs, the
...
typedef struct {
     ...
     * subcommands, openAMDC will also check allowed_subcommands in order to
...
typedef struct client {
    ...
    int iel;                /* The event loop id */
    struct mutex lock;      /* Client mutex lock */
    ...
    redisAtomic uint64_t flags; /* Client flags: CLIENT_* macros. */
    ...
    long long reploff_cmd;  /* The replication offset of the executing command */
    ...
    void *auth_module;      /* The module that owns the callback, which is used
                             ...
                             * unloaded for cleanup. Opaque for openAMDC Core.*/
...
/* The redisOp structure defines a openAMDC Operation, that is an instance of
...
/* Defines an array of openAMDC operations. There is an API to add to this
 ...
 * openAMDC build on AIX we need to undef it. */
#ifdef _AIX
#undef hz
#endif
...
struct redisServer {
    ...
    pthread_t thread[MAX_THREAD_VAR];
    ...
    int in_eval[MAX_THREAD_VAR]; /* Are we inside EVAL? */
    int in_exec[MAX_THREAD_VAR]; /* Are we inside EXEC? */
    int propagate_in_transaction[MAX_THREAD_VAR];  /* Make sure we don't propagate nested MULTI/EXEC */
    char *ignore_warnings;      /* Config: warnings that should be ignored. */
    int client_pause_in_transaction[MAX_THREAD_VAR]; /* Was a client pause executed during this Exec? */
    /* Modules */
    ...
    client *module_client;      /* "Fake" client to call openAMDC from modules */
    size_t moduleCount[MAX_THREAD_VAR]; /* Moudle count per event loop */
    /* Networking */
    ...
    socketFds ipfd[MAX_THREAD_VAR]; /* TCP socket file descriptors */
    socketFds tlsfd[MAX_THREAD_VAR]; /* TLS socket file descriptors */
    int sofd;                   /* Unix socket file descriptor */
    socketFds cfd;              /* Cluster bus listening socket */
    list *clients;              /* List of active clients */
    list *clients_to_close;     /* Clients to close asynchronously */
    list *pending_write_list[MAX_THREAD_VAR]; /* There is to write or install handler. */
    list *slaves, *monitors;    /* List of slaves and MONITORs */
    client *current_client[MAX_THREAD_VAR]; /* Current client executing the command. */
    rax *clients_timeout_table[MAX_THREAD_VAR]; /* Radix tree for blocked clients timeouts. */
    long fixed_time_expire[MAX_THREAD_VAR]; /* If > 0, expire keys against server.mstime. */
    int in_nested_call[MAX_THREAD_VAR]; /* If > 0, in a nested call of a call */
    rax *clients_index;         /* Active clients dictionary by client ID. */
    pause_type client_pause_type;      /* True if clients are currently paused */
    list *paused_clients;       /* List of pause clients */
    mstime_t client_pause_end_time;    /* Time when we undo clients_paused */
    char neterr[MAX_THREAD_VAR][ANET_ERR_LEN];   /* Error buffer for anet.c */
    dict *migrate_cached_sockets;/* MIGRATE cached sockets */
    redisAtomic uint64_t next_client_id; /* Next client unique ID. Incremental. */
    int protected_mode;         /* Don't accept external connections. */
    int gopher_enabled;         /* If true the server will reply to gopher
                                   queries. Will still serve RESP2 queries. */
    int worker_threads_num; /* Number of worker threads */
    long long events_processed_while_blocked; /* processEventsWhileBlocked() */
    redisAtomic int clients_num_in_thread[MAX_THREAD_VAR]; /* Client num in thread */
    list *client_handling_list[MAX_THREAD_VAR]; /* Client handling list */
    list *async_pending_write_list[MAX_THREAD_VAR]; /* Clients pending async write list */
    struct mutex pending_write_lock[MAX_THREAD_VAR]; /* Pending write lock */
    ...
    long long stat_total_error_replies[MAX_THREADS]; /* Total number of issued error replies ( command + rejected errors ) */
    ...
    /* Configuration */
    int verbosity;                  /* Loglevel in openamdc.conf */
    ...
    aeEventLoop *aof_pipe_read_ack_from_child_el;
    int aof_child_write_diff_installed;
    /* RDB persistence */
    ...
    int repl_transfer_iel;  /* Slave -> Master SYNC thread index */
    ...
    list *unblocked_clients[MAX_THREAD_VAR]; /* list of clients to unblock before next loop */
    ...
    int sort_bypattern;
    int sort_store;
    /* Zip structure config, see openamdc.conf for more information  */
    ...
    redisAtomic mstime_t mstime;            /* 'unixtime' in milliseconds. */
    redisAtomic ustime_t ustime;            /* 'unixtime' in microseconds. */
    ...
    int cluster_module_flags;      /* Set of flags that openAMDC modules are able
                                      to set in order to suppress certain
                                      native openAMDC Cluster features. Check the
    ...
    client *lua_client[MAX_THREAD_VAR];   /* The "fake client" to query openAMDC from Lua */
    ...
    /* cpu affinity */
    char *server_cpulist; /* cpu affinity list of openAMDC server main/io thread. */
...
struct redisCommand {
     ...
     * Used for openAMDC Cluster redirect. */
...
extern struct mutex asyncFreeQueueLock;
extern struct mutex replBacklogLock;
extern struct mutex expireLock;
...
int listenToPort(int port, socketFds *fds, int reusePort);
...
/* openAMDC object implementation */
