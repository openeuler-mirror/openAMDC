...
#else
        fprintf(stderr,"rediss:// is only supported when openamdc-cli is compiled with OpenSSL\n");
        exit(1);
#endif
...
static sds cliVersion(void) {
    sds version;
    version = sdscatprintf(sdsempty(), "%s", OPENAMDC_VERSION);
 ...
 * names from the help.h file. However the openAMDC instance we are connecting
 * to may support more commands, so this function integrates the previous
 * entries with additional entries obtained using the COMMAND command
 * available in recent versions of openAMDC. */
...
static void cliOutputGenericHelp(void) {
    ...
    printf(
        "openamdc-cli %s\n"
        "To get help about openAMDC commands type:\n"
        "      \"help @<group>\" to get a list of commands in <group>\n"
        "      \"help <command>\" for help on <command>\n"
        "      \"help <tab>\" to get a list of possible help topics\n"
        "      \"quit\" to exit\n"
        "\n"
        "To set openamdc-cli preferences:\n"
...
/* Select RESP3 mode if openamdc-cli was started with the -3 option.  */
...
static int cliConnect(int flags) {
    if (context == NULL || flags & CC_FORCE) {
        ...
        if (context->err) {
            if (!(flags & CC_QUIET)) {
                fprintf(stderr,"Could not connect to openAMDC at ");
                if (config.hostsocket == NULL ||
                    (config.cluster_mode && config.cluster_reissue_command))
                {
        ...
        /* Set aggressive KEEP_ALIVE socket option in the openAMDC context socket
...
static int cliSendCommand(int argc, char **argv, long repeat) {
    ...
    if (!config.eval_ldb && /* In debugging mode, let's pass "help" to openAMDC. */
    ...
    if (!strcasecmp(command,"info") ||
        ...
        (argc == 2 && !strcasecmp(command,"latency") &&
                       !strcasecmp(argv[1],"doctor")) ||
        /* Format PROXY INFO command for openAMDC Cluster Proxy:
...
static int parseOptions(int argc, char **argv) {
    ...
    for (i = 1; i < argc; i++) {
                   ...
                   && !lastarg)
        {
            ...
            config.memkeys = 1;
            config.memkeys_samples = 0; /* use openAMDC default */
...
"openamdc-cli %s\n"
"\n"
"Usage: openamdc-cli [OPTIONS] [cmd [arg [arg ...]]]\n"
...
"                     Default timeout: %d. Use 0 to wait forever.\n",
    ...
    fprintf(stderr,
"  --bigkeys          Sample openAMDC keys looking for keys with many elements (complexity).\n"
"  --memkeys          Sample openAMDC keys looking for keys consuming a lot of memory.\n"
"  --memkeys-samples <n> Sample openAMDC keys looking for keys consuming a lot of memory.\n"
"                     And define number of key elements to sample\n"
"  --hotkeys          Sample openAMDC keys looking for hot keys.\n"
...
"  --ldb              Used with --eval enable the openAMDC Lua debugger.\n"
...
"\n"
"Examples:\n"
"  cat /etc/passwd | openamdc-cli -x set mypasswd\n"
"  openamdc-cli get mypasswd\n"
"  openamdc-cli -r 100 lpush mylist x\n"
"  openamdc-cli -r 100 -i 1 info | grep used_memory_human:\n"
"  openamdc-cli --quoted-input set '\"null-\\x00-separated\"' value\n"
"  openamdc-cli --eval myscript.lua key1 key2 , arg1 arg2 arg3\n"
"  openamdc-cli --scan --pattern '*:12345*'\n"
"\n"
"  (Note: when using --eval the comma separates KEYS[] from ARGV[] items)\n"
"\n"
"When no command is given, openamdc-cli starts in interactive mode.\n"
...
void cliSetPreferences(char **argv, int argc, int interactive) {
    if (!strcasecmp(argv[0],":set") && argc >= 2) {
        if (!strcasecmp(argv[1],"hints")) pref.hints = 1;
            ...
            printf("%sunknown openamdc-cli preference '%s'\n",
                interactive ? "" : ".redisclirc: ",
                argv[1]);
        }
    } else {
        printf("%sunknown openamdc-cli internal command '%s'\n",
            interactive ? "" : ".redisclirc: ",
            argv[0]);
    }
}
            ...
            if (argv && argc > 0) {
                ...
                if (argc > 1 && *endptr == '\0') {
                    if (errno == ERANGE || errno == EINVAL || repeat <= 0) {
                        fputs("Invalid openamdc-cli repeat command option value.\n", stdout);
                        sdsfreesplitres(argv, argc);
                        linenoiseFree(line);
                        continue;
                    }
...
static sds clusterManagerGetNodeRDBFilename(clusterManagerNode *node) {
    ...
    filename = sdscatprintf(filename, "openamdc-node-%s-%d-%s.rdb", node->ip,
                            node->port, node->name);
    return filename;
}
...
/* Call "INFO" openAMDC command on the specified node and return the reply. */
 ...
 * specified nodes *n1 and *n2, by calling DEBUG DIGEST-VALUE openAMDC command
...
unhandled_case:
            success = 0;
            clusterManagerLogErr("[ERR] Sorry, openamdc-cli can't fix this slot "
...
/* Execute openAMDC-cli in Cluster Manager mode */
...
static int clusterManagerCommandCreate(int argc, char **argv) {
    ...
    if (masters_count < 3) {
        clusterManagerLogErr(
            ...
            "*** OpenAMDC Cluster requires at least 3 master nodes.\n"
...
static int clusterManagerCommandImport(int argc, char **argv) {
    ...
    if (src_ctx->err) {
        success = 0;
        fprintf(stderr,"Could not connect to openAMDC at %s:%d: %s.\n", src_ip,
            ...
            while(1) {
                ...
                if (obuf_len == 0 && !eof) {
                    ...
                    if (nread == 0) {
                         ...
                         * CRLF is ignored by openAMDC, so it has no effects. */
                        char echo[] =
