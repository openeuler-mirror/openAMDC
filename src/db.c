...
robj *lookupKey(redisDb *db, robj *key, int flags) {
    ...
    if (de) {
        ...
        if (server.swap_enabled &&
            cuckooFilterContains(&server.swap->cold_filter, key->ptr, sdslen(key->ptr))) {
            ...
            cuckooFilterDelete(&server.swap->cold_filter, key->ptr, sdslen(key->ptr));
...
robj *lookupKeyReadWithFlags(redisDb *db, robj *key, int flags) {
    ...
    if (expireIfNeeded(db,key) == 1) {
        ...
        if (server.current_client[threadId] &&
            server.current_client[threadId] != server.master &&
            server.current_client[threadId]->cmd &&
            server.current_client[threadId]->cmd->flags & CMD_READONLY)
        {
            goto keymiss;
        }
    }
...
/* Return a random key, in form of a openAMDC object.
...
/* This is a wrapper whose behavior depends on the openAMDC lazy free
...
/* Remove all keys from all the databases in a openAMDC server.
 * If callback is given the function is called from time to time to
 * signal that work is in progress.
 *
 * The dbnum can be -1 if all the DBs should be flushed, or the specified
 * DB number if we want to flush only a single openAMDC database number.
 *
...
long long emptyDb(int dbnum, int flags, void(callback)(void*)) {
    ...
    /* Empty openAMDC database structure. */
...
/* FLUSHDB [ASYNC]
 *
 * Flushes the currently SELECTed openAMDC DB. */
 ...
 * to fix all the references to the openAMDC DB structure.
 *
...
int keyIsExpired(redisDb *db, robj *key) {
    ...
    else if (server.fixed_time_expire[threadId] > 0) {
        now = server.mstime;
    }
    ...
    for (j = cmd->firstkey; j <= last; j += cmd->keystep) {
        if (j >= argc) {
            ...
            if (cmd->flags & CMD_MODULE || cmd->arity < 0) {
                ...
                serverPanic("openAMDC built-in command declared keys positions not matching the arity requirements.");
            }
        }
        keys[i++] = j;
    }
...
/* Slot to Key API. This is used by openAMDC Cluster in order to obtain in
...
/* Release the radix tree mapping openAMDC Cluster keys to slots. If 'async'
...
/* Empty the slots-keys map of openAMDC CLuster by creating a new empty one and
 * freeing the old one. */
