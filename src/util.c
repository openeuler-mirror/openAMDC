 ...
 * openAMDC: if it fails, strtoull() is used instead. The function returns
...
/* Generate the openAMDC "Run ID", a SHA1-sized random number that identifies a
 * given execution of openAMDC, so that if you are talking with an instance
 ...
 * environments where openAMDC runs. */
