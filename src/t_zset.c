 ...
 * data structure.
 *
 * The elements are added to a hash table mapping openAMDC objects to scores.
 * At the same time the elements are added to a skip list mapping scores
 * to openAMDC objects (so objects are sorted by scores in this "view").
 *
...
/* Parse max or min argument of ZRANGEBYLEX.
  ...
  *
  * If the string is valid the *dest pointer is set to the openAMDC object
...
static void zrangeResultFinalizeStore(zrange_result_handler *handler, size_t result_count)
{
    if (result_count) {
        ...
        if (dbDelete(handler->client->db, handler->dstkey)) {
            ...
            swapDel(handler->dstkey, handler->client->db->id);
            server.dirty++;
        }
    }
