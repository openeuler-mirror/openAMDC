 ...
 * opcode. New versions of openAMDC store using the RDB_OPCODE_EXPIRETIME_MS
 ...
 * (since it is a too large count to be applicable in any openAMDC data
 * structure). */
...
/* Like rdbSaveRawString() gets a openAMDC object instead. */
 ...
 * RDB_LOAD_ENC: If the returned type is a openAMDC object, try to
 *               encode it in a special way to be more memory
 *               efficient. When this flag is passed the function
 *               no longer guarantees that obj->ptr is an SDS string.
 * RDB_LOAD_PLAIN: Return a plain string allocated with zmalloc()
 *                 instead of a openAMDC object with an sds in it.
 * RDB_LOAD_SDS: Return an SDS string instead of a openAMDC object.
 *
...
int rdbSaveInfoAuxFields(rio *rdb, int rdbflags, rdbSaveInfo *rsi) {
    ...
    if (rdbSaveAuxFieldStrStr(rdb,"openamdc-ver",OPENAMDC_VERSION) == -1) return -1;
 ...
 * openAMDC I/O channel. On success C_OK is returned, otherwise C_ERR
...
int rdbSaveBackground(char *filename, rdbSaveInfo *rsi) {
    ...
    if ((childpid = redisFork(CHILD_TYPE_RDB)) == 0) {
        ...
        /* Child */
        redisSetProcTitle("openAMDC-rdb-bgsave");
 ...
 * a dummy openAMDC object is returned just to conform to the API. */
...
/* Load a openAMDC object of the specified type from the specified file.
...
robj *rdbLoadObject(int rdbtype, rio *rdb, sds key, int *error) {
    ...
    if (server.sanitize_dump_payload == SANITIZE_DUMP_CLIENTS) {
        ...
        int skip = server.loading ||
            (server.current_client[threadId] && (server.current_client[threadId]->flags & CLIENT_MASTER));
        if (!skip && server.current_client[threadId] && server.current_client[threadId]->user)
            skip = !!(server.current_client[threadId]->user->flags & USER_FLAG_SANITIZE_PAYLOAD_SKIP);
...
int rdbLoadRio(rio *rdb, int rdbflags, rdbSaveInfo *rsi) {
    ...
    while(1) {
        ...
        if (type == RDB_OPCODE_EXPIRETIME) {
            ...
            if (dbid >= (unsigned)server.dbnum) {
                serverLog(LL_WARNING,
                    "FATAL: Data file was created with a openAMDC "
            ...
            } else if (!strcasecmp(auxkey->ptr,"openamdc-ver")) {
            ...
            /* Load module data that is not related to the openAMDC key space.
     ...
     * this will in turn either abort openAMDC in most cases, or if we are loading
...
void asyncDeleteFileEvent(void *var, aeAsyncCallback *callback) {
    UNUSED(callback);
    int rdb_pipe_read = (int)((int64_t)var);
    aeDeleteFileEvent(server.el[threadId], rdb_pipe_read, AE_READABLE);
    close(rdb_pipe_read);
}
...
static void backgroundSaveDoneHandlerSocket(int exitcode, int bysignal) {
    ...
    void *var = (void*)((int64_t)server.rdb_pipe_read);
    aeAsyncFunction(server.el[MAIN_THREAD_ID], asyncDeleteFileEvent, var, NULL, 1);
...
/* Asynchronous setup handler for reading from the RDB pipe */
void asyncRdbPipeReadHandler(void *var, aeAsyncCallback *callback) {
    UNUSED(callback);
    UNUSED(var);
...
int rdbSaveToSlavesSockets(rdbSaveInfo *rsi) {
    ...
    if ((childpid = redisFork(CHILD_TYPE_RDB)) == 0) {
        ...
        redisSetProcTitle("openAMDC-rdb-to-slaves");
        redisSetCpuAffinity(server.bgsave_cpulist);
        ...
        if (childpid == -1) {
            ...
            aeAsyncFunction(server.el[MAIN_THREAD_ID], asyncRdbPipeReadHandler, NULL, NULL, 1);
        }
