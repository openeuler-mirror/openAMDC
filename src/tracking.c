 ...
 * to the keys the user fetched, so that openAMDC will know what are the clients
...
/* This function is called when a key is modified in openAMDC and in the case
...
/* This function is called from signalModifiedKey() or other places in openAMDC
 ...
 * the function is called from the openAMDC core once a key is modified, however
...
void trackingInvalidateKey(client *c, robj *keyobj, int bcast) {
    ...
    while(raxNext(&ri)) {
        ...
        if (target->flags & CLIENT_TRACKING_NOLOOP &&
            target == server.current_client[threadId])
        {
            continue;
        }
        ...
        if (target == server.current_client[threadId] && server.fixed_time_expire[threadId]){
            incrRefCount(keyobj);
...
void trackingHandlePendingKeyInvalidations() {
    ...
    while ((ln = listNext(&li)) != NULL) {
        ...
        if (server.current_client[threadId] != NULL) {
            if (key != NULL) {
                sendTrackingMessage(server.current_client[threadId],(char *)key->ptr,sdslen(key->ptr),0);
            } else {
                sendTrackingMessage(server.current_client[threadId],shared.null[server.current_client[threadId]->resp]->ptr,
                    sdslen(shared.null[server.current_client[threadId]->resp]->ptr),1);
            }
        }
...
/* This function is called when one or all the openAMDC databases are
...
/* Tracking forces openAMDC to remember information about which client may have
 ...
 *
 * So openAMDC allows the user to configure a maximum number of keys for the
