...
struct mutex asyncFreeQueueLock;
...
void linkClient(client *c) {
    ...
    if (c->conn != NULL) server.clients_num_in_thread[c->iel]++;
...
client *createClient(connection *conn, int iel) {
    serverAssert(conn == NULL || (iel == threadId));
    ...
    if (conn) {
        serverAssert(iel == threadId);
        connNonBlock(conn);
        connEnableTcpNoDelay(conn);
        if (server.tcpkeepalive)
            connKeepAlive(conn,server.tcpkeepalive);
        connSetReadHandler(conn, readQueryFromClient, 1);
    ...
    c->argv = NULL;
    c->argv_len_sum = 0;
    c->parsed_list = listCreate();
    listSetFreeMethod(c->parsed_list, parsedRelease);
    c->original_argc = 0;
...
/* Determines if a client's event processing is associated with
 * the correct thread */
int clientCheckThreadAligned(client *c) {
    /* Check if the client connection is NULL, indicating no
     * specific thread binding */
    if (c->conn == NULL) return 1;
    
    /* Check if the client is bound to the main worker thread and
      * we're in the designated module thread */
    if (c->iel == MAIN_THREAD_ID && threadId == MODULE_THREAD_ID) return 1;

    /* Check if the client's threadId matches the current threadId
     * (excluding the default -1) */
    if (threadId != -1 && c->iel == threadId) return 1;

    /* In scenarios not meeting the above conditions, the client-event
     * processing alignment is incorrect. */
    return 0;
}

/* Asynchronous function handler that executes a provided callback
 * under a wrapper mutex lock */
void callbackFuntion(void *var, aeAsyncCallback *callback) {
    client *c = var;

    /* Lock the client's mutex using the wrapper function to
    ...
    /* Decrease the count of pending asynchronous operations
     * for the client */
    c->async_ops--;
}
...
int parseClientCommandBuffer(client *c, int callFlags) {
    int ret = C_OK;
    while (c->qb_pos < sdslen(c->querybuf)) {
        ...
        if (c->flags & CLIENT_INPROGRESS_COMMAND) {
            ret = C_ERR;
            break;
        }
        ...
        * The same applies for clients we want to terminate ASAP. */
        ...
        if (c->reqtype == PROTO_REQ_INLINE) {
             ...
             * the request in Gopher mode. To avoid data race, openAMDC won't
             * support Gopher if enable io threads to read queries. */
            if (server.gopher_enabled && !server.worker_threads_num &&
        ...
        if (listLength(c->parsed_list)) {
            listNode *ln = listLast(c->parsed_list);
            parsed *p = listNodeValue(ln);
            if (p && (p->argc <= 0 || p->argv == NULL)) {
                listDelNode(c->parsed_list, ln);
            } else {
                if (c->flags & CLIENT_MASTER) {
                    p->reploff = c->read_reploff - sdslen(c->querybuf) + c->qb_pos;
                }
                serverAssert(p->reploff >= 0);
            }
        }
...
/* Installs an asynchronous write handler for a client, if not already installed */
void clientInstallAsyncWriteHandler(client *c) {
    serverAssert(threadOwnLock());
    if (!c->async_write_handler_active) {
        c->async_write_handler_active = 1;
        listAddNodeHead(server.async_pending_write_list[threadId], c);
    }
}
...
int prepareClientToWrite(client *c) {
    int async = !clientCheckThreadAligned(c);
    if (async) {
        serverAssert(threadOwnLock());
    } else {
        WRAPPER_MUTEX_NOCLEANUP_DEFINE(cl, &c->lock);
     ...
     */
    if (!async && !clientHasPendingReplies(c))
        clientInstallWriteHandler(c);
    if (async && !c->async_write_handler_active)
...
int _addReplyToBuffer(client *c, const char *s, size_t len) {
    ...
    if (clientCheckThreadAligned(c)) {
        ...
        * add anything more to the static buffer. */
        ...
        /* Initializes or resizes the async reply block to accommodate the new data */
        resizeAsyncReplyBlockIfNeeded(c, len);

        /* Copies the reply data into the async reply block and updates the used size */
        memcpy(c->async_reply_block->buf + c->async_reply_block->used, s, len);
        c->async_reply_block->used += len;
    }
 ...
 *
 * It is efficient because does not create an SDS object nor an openAMDC object
 ...
 * It emits the protocol for a openAMDC error, in the form:
 *
...
void afterErrorReply(client *c, const char *s, size_t len) {
    ...
    server.stat_total_error_replies[threadId]++;
     ...
     * they are rare and may hint at errors in a script or a bug in openAMDC. */
...
void *addReplyDeferredLen(client *c) {
    ...
    if (clientCheckThreadAligned(c)) {
        ...
        return (void *)((ssize_t)(c->async_reply_block ? c->async_reply_block->used : 0));
    }
}
...
#define LENSTR_BUFFER_SIZE 128
/* Sets the deferred aggregate length for an aligned client */
void setDeferredAggregateLenAlignedClient(client *c, void *node, long length, char prefix) {
    /* Abort when *node is NULL: when the client should not accept writes
    * we return NULL in addReplyDeferredLen() */
    if (node == NULL) return;
    ...
    * like it is most of the times. */
    ...
    if (opt_hdr) {
        switch (prefix) {
            case '*': setDeferredReply(c, node, shared.mbulkhdr[length]->ptr, hdr_len); return;
            case '%': setDeferredReply(c, node, shared.maphdr[length]->ptr, hdr_len); return;
            case '~': setDeferredReply(c, node, shared.sethdr[length]->ptr, hdr_len); return;
            default: break;
        }  
    }

    /* Construct the length header string */
    char lenstr[LENSTR_BUFFER_SIZE];
    size_t lenstr_len = sprintf(lenstr, "%c%ld\r\n", prefix, length);
    setDeferredReply(c, node, lenstr, lenstr_len);
}

/* Sets the deferred aggregate length for an unaligned client */
void setDeferredAggregateLenUnAlignedClient(client *c, void *node, long length, char prefix) {
    /* Construct the length header string */
    char lenstr[LENSTR_BUFFER_SIZE];
    int lenstr_len = snprintf(lenstr, sizeof(lenstr), "%c%ld\r\n", prefix, length);

    /* Use the node pointer as an index into the async reply block */
    size_t async_reply_block_use = (size_t)node;
    serverAssert(async_reply_block_use <= c->async_reply_block->used);

    /* Ensure the async reply block has enough capacity; resize if needed */
    if (c->async_reply_block->size < (c->async_reply_block->used + lenstr_len)) {
        clientReplyBlock *new = zmalloc(sizeof(clientReplyBlock) + c->async_reply_block->used + lenstr_len);
        new->size = zmalloc_usable_size(new) - sizeof(clientReplyBlock);
        new->used = c->async_reply_block->used;
        memcpy(new->buf, c->async_reply_block->buf, c->async_reply_block->used);
        freePendingAsyncReply(c);
        c->async_reply_block = new;
    }

    /* Shift existing data to make room for the new header and insert it */
    memmove(c->async_reply_block->buf + async_reply_block_use + lenstr_len,
            c->async_reply_block->buf + async_reply_block_use,
            c->async_reply_block->used - async_reply_block_use);
    memcpy(c->async_reply_block->buf + async_reply_block_use, lenstr, lenstr_len);

    /* Update the used size of the async reply block */
    c->async_reply_block->used += lenstr_len;
}
...
void setDeferredAggregateLen(client *c, void *node, long length, char prefix) {
    ...
    if (clientCheckThreadAligned(c)) {
       setDeferredAggregateLenAlignedClient(c, node, length, prefix);
    } else {
        setDeferredAggregateLenUnAlignedClient(c, node, length, prefix);
    }
}
...
/* Add a openAMDC Object as a bulk reply */
...
void clientAcceptHandler(connection *conn) {
        ...
        !(c->flags & CLIENT_UNIX_SOCKET))
    {
        ...
        if (strcmp(cip,"127.0.0.1") && strcmp(cip,"::1")) {
            char *err =
                "-DENIED openAMDC is running in protected mode because protected "
                "mode is enabled, no bind address was specified, no "
                "authentication password is requested to clients. In this mode "
                "connections are only accepted from the loopback interface. "
                "If you want to connect from external computers to openAMDC you "
                "may adopt one of the following solutions: "
                "1) Just disable protected mode sending the command "
                "'CONFIG SET protected-mode no' from the loopback interface "
                "by connecting to openAMDC from the same host the server is "
                "running, however MAKE SURE openAMDC is not publicly accessible "
                "from internet if you do so. Use CONFIG REWRITE to make this "
                "change permanent. "
                "2) Alternatively you can just disable the protected mode by "
                "editing the openAMDC configuration file, and setting the protected "
...
void acceptTcpHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while(max--) {
        cfd = anetTcpAccept(server.neterr[threadId], fd, cip, sizeof(cip), &cport);
        if (cfd == ANET_ERR) {
            if (errno != EWOULDBLOCK)
                serverLog(LL_WARNING,
                    "Accepting client connection: %s", server.neterr[threadId]);
            return;
        }
        anetCloexec(cfd);
        serverLog(LL_VERBOSE,"Accepted %s:%d", cip, cport);
        WRAPPER_MUTEX_LOCK(gl, &globalLock);
        acceptCommonHandler(connCreateAcceptedSocket(cfd),0,cip,threadId);
    }
}
...
void acceptTLSHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while(max--) {
        ...
        serverLog(LL_VERBOSE,"Accepted %s:%d", cip, cport);
        WRAPPER_MUTEX_LOCK(gl, &globalLock);
        acceptCommonHandler(connCreateAcceptedTLS(cfd, server.tls_auth_clients),0,cip,threadId);
    }
}
...
void acceptUnixHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while(max--) {
        cfd = anetUnixAccept(server.neterr[threadId], fd);
        if (cfd == ANET_ERR) {
            if (errno != EWOULDBLOCK)
                serverLog(LL_WARNING,
                    "Accepting client connection: %s", server.neterr[threadId]);
        ...
        WRAPPER_MUTEX_LOCK(gl, &globalLock);
        acceptCommonHandler(connCreateAcceptedSocket(cfd),CLIENT_UNIX_SOCKET,NULL,threadId);
    }
}
...
void unlinkClient(client *c) {
    ...
    if (threadId != -1 && server.current_client[threadId] == c) server.current_client[threadId] = NULL;
    ...
    if (c->conn) {
        ...
        connClose(c->conn);
        c->conn = NULL;
        server.clients_num_in_thread[c->iel]--;
    }

    /* Remove from the list of pending writes if needed. */
    if (c->flags & CLIENT_PENDING_WRITE) {
        WRAPPER_MUTEX_LOCK(pwl, &server.pending_write_lock[c->iel]);
        ln = listSearchKey(server.pending_write_list[c->iel],c);
        serverAssert(ln != NULL);
        listDelNode(server.pending_write_list[c->iel],ln);
        c->flags &= ~CLIENT_PENDING_WRITE;
    }
    ...
    if (c->flags & CLIENT_UNBLOCKED) {
        ln = listSearchKey(server.unblocked_clients[c->iel],c);
        serverAssert(ln != NULL);
        listDelNode(server.unblocked_clients[c->iel],ln);
    ...
    if (c->async_write_handler_active) {
        ...
        for (int iel = 0; iel < MAX_THREAD_VAR; ++iel) {
            if (iel < server.worker_threads_num || 
                ...
                if (ln) {
                    found = 1;
                    listDelNode(server.async_pending_write_list[iel], ln);
                }
            }
        }
        serverAssert(found);
    ...
    listRewind(server.client_handling_list[c->iel], &li);
    while((ln = listNext(&li))) {
        client *client = listNodeValue(ln);
        if (client == c) {
            listDelNode(server.client_handling_list[c->iel], ln);
        }
    }
...
int freeClientsInAsyncFreeQueue(int iel) {
    ...
    while ((ln = listNext(&li)) != NULL) {
        ...
        if (c->iel == iel && !(c->flags & CLIENT_PROTECTED) && !c->async_ops) { 
            c->flags &= ~CLIENT_CLOSE_ASAP;
            freeClient(c);
            listDelNode(server.clients_to_close,ln);
            freed++;
...
/* Handles the logic of processing the async reply buffer */
void handleAsyncReplyBuffer(client *c) {
    if (c->async_reply_block != NULL) {
        size_t size = c->async_reply_block->used;
        if (listLength(c->reply) == 0 &&
            size <= (size_t)(PROTO_REPLY_CHUNK_BYTES - c->bufpos)) {
            memcpy(c->buf + c->bufpos, c->async_reply_block->buf, size);
            c->bufpos += size;
        } else {
            c->reply_bytes += c->async_reply_block->size;
            listAddNodeTail(c->reply, c->async_reply_block);
            c->async_reply_block = NULL;
        }
        freePendingAsyncReply(c);
    } else {
...
/* Helper function to free the pending async reply buffer if it exists */
void freePendingAsyncReply(client *c) {
    if (c->async_reply_block != NULL) {
        zfree(c->async_reply_block);
        c->async_reply_block = NULL;
    }
}

/* Helper function to resize the async reply block if necessary */
void resizeAsyncReplyBlockIfNeeded(client *c, size_t len) {
    /* Checks if the block needs to be initialized or resized based on
       available space vs. required space */
    if (c->async_reply_block == NULL || 
        (c->async_reply_block->size - c->async_reply_block->used) < len) {
        size_t newSize;
        if (c->async_reply_block)
            newSize = c->async_reply_block->used + len > c->async_reply_block->size * 2 ?
                c->async_reply_block->used + len : c->async_reply_block->size * 2;
        else
            newSize = len > PROTO_ASYNC_REPLY_CHUNK_BYTES ? len : PROTO_ASYNC_REPLY_CHUNK_BYTES;

        /* Allocates memory for the new block */
        clientReplyBlock *newBlock = zmalloc(sizeof(clientReplyBlock) + newSize);
        newBlock->size = zmalloc_usable_size(newBlock) - sizeof(clientReplyBlock);
        newBlock->used = c->async_reply_block ? c->async_reply_block->used : 0;

        /* If an old block exists, it's data is copied to the new block before
           freeing the old block */
        if (c->async_reply_block) {
            memcpy(newBlock->buf, c->async_reply_block->buf, c->async_reply_block->used);
            freePendingAsyncReply(c);
        }

        /* Assigns the new block to the client's async_reply_block */
        c->async_reply_block = newBlock;
    }
}
...
int writeToClient(client *c, int handler_installed) {
    ...
    if (!clientHasPendingReplies(c)) {
         ...
         * so we are fine. */
        if (handler_installed) connSetWriteHandler(c->conn, NULL, 0);
...
int handleClientsWithPendingWrites(int iel) {
    ...
    WRAPPER_MUTEX_LOCK(pwl, &server.pending_write_lock[threadId]);
    int processed = listLength(server.pending_write_list[threadId]);    
    listRewind(server.pending_write_list[threadId],&li);
    while((ln = listNext(&li))) {
        client *c = listNodeValue(ln);
        c->flags &= ~CLIENT_PENDING_WRITE;
        listDelNode(server.pending_write_list[threadId],ln);

        /* If a client is protected, don't do anything,
         * that may trigger write error or recreate handler. */
        if ((c->flags & CLIENT_PROTECTED) && !(c->flags & CLIENT_SLAVE)) continue;
        ...
        if (clientHasPendingReplies(c)) {
            ...
            if (connSetWriteHandlerWithBarrier(c->conn, sendReplyToClient, ae_barrier, 1) == C_ERR) {
                freeClientAsync(c);
            }
        }
    }
...
void protectClient(client *c) {
    ...
    if (c->conn) {
        connSetReadHandler(c->conn,NULL,0);
        connSetWriteHandler(c->conn,NULL,0);
    }
}
...
void unprotectClient(client *c) {
    ...
    if (c->flags & CLIENT_PROTECTED) {
        c->flags &= ~CLIENT_PROTECTED;
        if (c->conn) {
            connSetReadHandler(c->conn,readQueryFromClient, 1);
...
int processInlineBuffer(client *c) {
     ...
     * commands. If this happens, it is likely due to a bug in openAMDC where
    ...
    /* Create openAMDC objects for all arguments. */
    if (argc) {
        parsed *p = parsedCreate(argc);
        for (j = 0; j < argc; j++) {
            p->argv[p->argc++] = createObject(OBJ_STRING, argv[j]);
            p->argv_len_sum += sdslen(argv[j]);
        }
        listAddNodeTail(c->parsed_list, p);
    }
    zfree(argv);
    return C_OK;
}
...
int processMultibulkBuffer(client *c) {
    ...
    if (c->multibulklen == 0) {
        ...
        /* Setup argv array on client structure */
        listAddNodeTail(c->parsed_list, parsedCreate(ll));
    }
    ...
    while(c->multibulklen) {
        ...
        if (sdslen(c->querybuf)-c->qb_pos < (size_t)(c->bulklen+2)) {
            ...
            {
                p->argv[p->argc++] = createObject(OBJ_STRING,c->querybuf);
                p->argv_len_sum += c->bulklen;
...
void commandProcessed(client *c, int callFlags) {
    UNUSED(callFlags);
...
int processCommandAndResetClient(client *c, int callFlags) {
    serverAssert(threadOwnLock());
    int deadclient = 0;
    client *old_client = server.current_client[threadId];
    server.current_client[threadId] = c;
    c->flags |= CLIENT_INPROGRESS_COMMAND;
    if (processCommand(c, callFlags) == C_OK) {
        commandProcessed(c, callFlags);
    }
    c->flags &= ~CLIENT_INPROGRESS_COMMAND;
    if (server.current_client[threadId] == NULL) deadclient = 1;
    /*
     ...
     */
    server.current_client[threadId] = old_client;
...
int processPendingCommandsAndResetClient(client *c, int callFlags) {
    if (c->flags & CLIENT_PENDING_COMMAND) {
...
void processInputBuffer(client *c, int parse, int callFlags) {
    serverAssert(clientCheckThreadAligned(c));
    if (parse) {
        if(parseClientCommandBuffer(c, callFlags) != C_OK)
            return;
    }
...
/* This function is a critical piece of the server's command processing logic,
 * ensuring that commands are properly executed and the server state is maintained. */
void processParsedList(client *c, int callFlags) {
    /* Keep processing while there is something in the input buffer */
    while(listLength(c->parsed_list)) {
        /* Return if we're still parsing this command */
        listNode *ln = listFirst(c->parsed_list);
        parsed *p = listNodeValue(ln);

        if (p->argc != p->max_argc)
            break;
...
/* Reads a query from a client connection and processes it. */
void readQueryFromClient(connection *conn) {
    /* Retrieve the client data associated with the connection. */
    client *c = connGetPrivateData(conn);
    int nread, readlen, ret;
    size_t qblen;

    /* Assert that the client is aligned with the current thread. */
    serverAssert(clientCheckThreadAligned(c));

    /* Define and lock the client's mutex for exclusive access. */
    WRAPPER_MUTEX_DEFER_LOCK(cl, &c->lock);
    ret = wrapperMutexTryLock(&cl);
     ...
     * openAMDC Object representing the argument. */
    ...
    /* Check if the system is running with multiple worker threads. */
    if (server.worker_threads_num > 1) {
        /* Attempt to parse the command buffer for the client. */
        if(parseClientCommandBuffer(c, CMD_CALL_FULL) == C_OK) {
            if (listLength(c->parsed_list)) {
                listAddNodeTail(server.client_handling_list[threadId], c);
            }
        }
    } else {
        /* Lock the global mutex and process the input buffer directly. */
        WRAPPER_MUTEX_LOCK(gl, &globalLock);
        processInputBuffer(c, 1, CMD_CALL_FULL);
    }
}
...
/* A openAMDC "Address String" is a colon separated ip:port pair.
 * For IPv4 it's in the form x.y.z.k:port, example: "127.0.0.1:1234".
 * For IPv6 addresses we use [] around the IP part, like in "[::1]:1234".
 * For Unix sockets we use path:0, like in "/tmp/openAMDC:0".
 *
...
sds getAllClientsInfoString(int type) {
    ...
    while ((ln = listNext(&li)) != NULL) {
        ...
        WRAPPER_MUTEX_LOCK(cl, &client->lock);
...
/* Asynchronous function to free clients queued for asynchronous deallocation
 * on a specific thread */
void asyncFreeClientsInAsyncFreeQueue(void *var, aeAsyncCallback *callback) {
    UNUSED(callback);
...
NULL
        ...
        while ((ln = listNext(&li)) != NULL) {
            ...
            if (c == client) {
                ...
                if (clientCheckThreadAligned(client)) {
                    freeClient(client);
                } else {
                    void *var = (void*)((int64_t)client->iel);
                    freeClientAsync(client);
                    aeAsyncFunction(server.el[client->iel],
                        asyncFreeClientsInAsyncFreeQueue, var, NULL, 1);
                }
            }
            killed++;
        }
                                                          ...
                                                          c->argc == 4))
    {
        ...
        if (target && target->flags & CLIENT_BLOCKED && moduleBlockedClientMayTimeout(target)) {
            WRAPPER_MUTEX_LOCK(tl, &target->lock);
            if (unblock_error)
...
void helloCommand(client *c) {
    ...
    addReplyBulkCString(c,OPENAMDC_VERSION);
 ...
 * openAMDC instances via HTTP, with a technique called "cross protocol scripting"
 * which exploits the fact that services like openAMDC will discard invalid
 * HTTP headers and will process what follows.
 *
 * As a protection against this attack, openAMDC will terminate the connection
...
void securityWarningCommand(client *c) {
    ...
    if (llabs(now-logged_time) > 60) {
        serverLog(LL_WARNING,"Possible SECURITY ATTACK detected. It looks like somebody is sending POST or Host: commands to openAMDC. This is likely due to an attacker attempting to use Cross Protocol Scripting to compromise your openAMDC instance. Connection aborted.");
        logged_time = now;
    }
    freeClientAsync(c);
}
...
unsigned long getClientOutputBufferMemoryUsage(client *c) {
    ...
    return c->reply_bytes + (list_item_size*listLength(c->reply)) +
        (c->async_reply_block ? c->async_reply_block->size : 0);
}
...
void flushSlavesOutputBuffers(void) {
    ...
    while((ln = listNext(&li))) {
        ...
        if (!clientCheckThreadAligned(slave)) continue;
 ...
 * 
 * This function is also internally used by openAMDC Cluster for the manual
...
void pauseClients(mstime_t end, pause_type type) {
     ...
     * in propagate(). */
    if (server.in_exec[threadId]) {
        server.client_pause_in_transaction[threadId] = 1;
    }
}
...
/* This function is called by openAMDC in order to process a few events from
...

    {
        ...
        * See https://github.com/redis/redis/issues/6988 for more info. */
        ...
        while (iterations--) {
            ...
            long long ae_events = aeProcessEvents(server.el[iel],
