...
void moduleHandlePropagationAfterCommandCallback(RedisModuleCtx *ctx) {
     ...
     * a transaction. */
    if (!server.propagate_in_transaction[threadId]) return;

    /* If this command is executed from with Lua or MULTI/EXEC we do not
     * need to propagate EXEC */
    if (server.in_eval[threadId] || server.in_exec[threadId]) return;
...
RedisModuleCallReply *RM_Call(RedisModuleCtx *ctx, const char *cmdname, const char *fmt, ...) {
    ...
    if (server.module_client == NULL) {
        ...
        c = server.module_client = createClient(NULL, MAIN_THREAD_ID);
        ...
        c = createClient(NULL, MAIN_THREAD_ID);
    }
...
RedisModuleBlockedClient *moduleBlockClient(RedisModuleCtx *ctx, RedisModuleCmdFunc reply_callback, RedisModuleCmdFunc timeout_callback, void (*free_privdata)(RedisModuleCtx*,void*), long long timeout_ms, RedisModuleString **keys, int numkeys, void *privdata) {
    client *c = ctx->client;
    int islua = server.in_eval[threadId];
    int ismulti = server.in_exec[threadId];
    ...
    bc->reply_client = createClient(NULL, MAIN_THREAD_ID);
...
RedisModuleCtx *RM_GetThreadSafeContext(RedisModuleBlockedClient *bc) {
     ...
     * things. */
    ctx->client = createClient(NULL, MAIN_THREAD_ID);
    if (bc) {
...
RedisModuleCtx *RM_GetDetachedThreadSafeContext(RedisModuleCtx *ctx) {
    ...
    new_ctx->client = createClient(NULL, MAIN_THREAD_ID);
    return new_ctx;
}
 ...
 *
 * Notice that it is not necessary to enable notifications in openamdc.conf for
...
/* Asynchronously creates or updates a timer event in the main worker thread */
void asyncCreateTimer(void *var, aeAsyncCallback *callback) {
    UNUSED(callback);
    struct asyncParameters *p = var;
    ...
    if (aeTimer != -1) {
        ...
        raxSeek(&ri,"^",NULL,0);
        raxNext(&ri);
        if (memcmp(ri.key,&p->key,sizeof(p->key)) == 0) {
            /* This is the first key, we need to re-install the timer according
             * to the just added event. */
            aeDeleteTimeEvent(server.el[MAIN_THREAD_ID],aeTimer);
            aeTimer = -1;
        }
        raxStop(&ri);
    }
    ...
    if (aeTimer == -1)
        aeTimer = aeCreateTimeEvent(server.el[MAIN_THREAD_ID],p->period,moduleTimerHandler,NULL,NULL);
...
RedisModuleTimerID RM_CreateTimer(RedisModuleCtx *ctx, mstime_t period, RedisModuleTimerProc callback, void *data) {
    ...
    struct asyncParameters *p = zmalloc(sizeof(struct asyncParameters));
    p->key = key;
    p->period = period;
    aeAsyncFunction(server.el[MAIN_THREAD_ID], asyncCreateTimer, p, NULL, 1);
...
void revokeClientAuthentication(client *c) {
    ...
    if (c == server.current_client[threadId]) {
...
void moduleInitModulesSystemLast(void) {
    moduleFreeContextReusedClient = createClient(NULL, MAIN_THREAD_ID);
