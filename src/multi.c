...
void beforePropagateMulti() {
    ...
    server.propagate_in_transaction[threadId] = 1;
}

void afterPropagateExec() {
    /* Propagating EXEC */
    serverAssert(server.propagate_in_transaction[threadId] == 1);
    server.propagate_in_transaction[threadId] = 0;
}
...
void execCommand(client *c) {
    ...
    if (server.propagate_in_transaction[threadId]) {
 ...
 * as in order to identify a key in openAMDC we need both the key name and the
 * DB */
