...
void clusterInit(void) {
    ...
    if (listenToPort(port+CLUSTER_PORT_INCR, &server.cfd, 0) == C_ERR) {
...
/* Asynchronously frees a cluster link object */
void asyncFreeClusterLink(void *var, aeAsyncCallback *callback) {
    UNUSED(callback);
    clusterLink *link = var;
...
void freeClusterLink(clusterLink *link) {
    if (threadId != MAIN_THREAD_ID) {
        if (link->node)
            link->node->link = NULL;
        link->node = NULL;
        int res = aeAsyncFunction(server.el[MAIN_THREAD_ID], asyncFreeClusterLink, link, NULL, 1);
...
static void clusterConnAcceptHandler(connection *conn) {
    ...
    connSetReadHandler(conn, clusterReadHandler, 0);
}
...
void clusterAcceptHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while(max--) {
        ...
        if (cfd == ANET_ERR) {
            if (errno != EWOULDBLOCK)
                serverLog(LL_VERBOSE,
                    "Error accepting cluster node: %s", server.neterr[threadId]);
            return;
        }
...
int clusterProcessPacket(clusterLink *link) {
    serverAssert(threadId == MAIN_THREAD_ID);
...
void clusterWriteHandler(connection *conn) {
    ...
    if (sdslen(link->sndbuf) == 0)
        connSetWriteHandler(link->conn, NULL, 0);
}
...
/* Asynchronously sets a write handler with a barrier for a cluster connection,
 * ensuring controlled writes. */
void asyncSetWriteHandlerWithBarrier(void *var, aeAsyncCallback *callback) {
    UNUSED(callback);
    clusterLink *link = var;
    if (link->conn != NULL) {
        connSetWriteHandlerWithBarrier(link->conn, clusterWriteHandler, 1, 0);
    }
}
...
void clusterSendMessage(clusterLink *link, unsigned char *msg, size_t msglen) {
    ...
    if (sdslen(link->sndbuf) == 0 && msglen != 0) {
        aeAsyncFunction(server.el[MAIN_THREAD_ID], asyncSetWriteHandlerWithBarrier, link, NULL, 1);
    }
 ...
 *
 * In openAMDC Cluster pongs are not used just for failure detection, but also
...
void clusterSendFailoverAuthIfNeeded(clusterNode *node, clusterMsg *request) {
     ...
     * right to vote, as the cluster size in openAMDC Cluster is the number
        ...
        if (node->link == NULL) {
            ...
            if (connConnect(link->conn, node->ip, node->cport, NET_FIRST_BIND_ADDR,
                ...
                serverLog(LL_DEBUG, "Unable to connect to "
                    ...
                    node->cport, server.neterr[threadId]);
...
/* Verify that the RDB version of the dump payload matches the one of this openAMDC
...
/* DUMP keyname
 * DUMP is actually not used by openAMDC Cluster but it is the obvious
...
try_again:
    ...
    if (!copy) {
         ...
         * from the receiving openAMDC server, by using the del_idx index. */
        if (del_idx > 1) {
 ...
 * the target instance. See the openAMDC Cluster specification for more
 * information. */
...
clusterNode *getNodeByQuery(client *c, struct redisCommand *cmd, robj **argv, int argc, int *hashslot, int *error_code) {
    ...
    /* Modules can turn off openAMDC Cluster redirection: this is useful
